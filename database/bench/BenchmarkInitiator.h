...
class BenchmarkInitiator {
   public:
    BenchmarkInitiator(const size_t& thread_count, ClusterConfig* config)
        : thread_count_(thread_count), config_(config) {}
    ...
    GAddr InitMonotoneTimestamp() {
        ...
        if (config_->IsMaster()) {
            monotome_ts_addr =
                default_gallocator->AlignedMalloc(sizeof(uint64_t));
            uint64_t ts = 1;
