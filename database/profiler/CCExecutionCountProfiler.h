...
#define INIT_CC_EXECUTION_COUNT_PROFILER \
    cc_execution_count_ = new std::unordered_map<size_t, size_t>[gThreadCount];

#define UPDATE_CC_EXECUTION_COUNT(thread_id, point_id)   \
    if (cc_execution_count_[thread_id].find(point_id) == \
        cc_execution_count_[thread_id].end()) {          \
...
#define REPORT_CC_EXECUTION_COUNT_PROFILER                                     \
    ...
    for (int i = 0; i < gThreadCount; ++i) {                                   \
        std::map<size_t, size_t> ordered_count(cc_execution_count_[i].begin(), \
                                               cc_execution_count_[i].end());  \
        for (auto &entry_count : ordered_count) {                              \
            printf("thread_id = %d, point_id = %d, count = %d\n", i,           \
                   (int)entry_count.first, (int)entry_count.second);           \
        }                                                                      \
    }                                                                          \
