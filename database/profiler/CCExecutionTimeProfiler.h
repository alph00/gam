...
#define INIT_CC_EXECUTION_TIME_PROFILER                            \
    cc_execution_time_stat_ =                                      \
        new std::unordered_map<size_t, long long>[gThreadCount];   \
    cc_execution_timer_ = new PreciseTimeMeasurer *[gThreadCount]; \
...
#define INIT_CC_EXECUTION_TIME_PROFILER                          \
    cc_execution_time_stat_ =                                    \
        new std::unordered_map<size_t, long long>[gThreadCount]; \
    cc_execution_timer_ = new TimeMeasurer *[gThreadCount];      \
    for (size_t i = 0; i < gThreadCount; ++i)                    \
...
#define END_CC_EXECUTION_TIME_MEASURE(thread_id, point_id)                    \
    ...
    if (cc_execution_time_stat_[thread_id].find(point_id) ==                  \
        cc_execution_time_stat_[thread_id].end()) {                           \
        cc_execution_time_stat_[thread_id][point_id] =                        \
            cc_execution_timer_[thread_id][point_id].GetElapsedNanoSeconds(); \
    } else {                                                                  \
        cc_execution_time_stat_[thread_id][point_id] +=                       \
            cc_execution_timer_[thread_id][point_id].GetElapsedNanoSeconds(); \
    }
...
#define REPORT_CC_EXECUTION_TIME_PROFILER                                     \
    ...
    for (int i = 0; i < gThreadCount; ++i) {                                  \
        std::map<size_t, long long> ordered_stat(                             \
            cc_execution_time_stat_[i].begin(),                               \
            cc_execution_time_stat_[i].end());                                \
...
enum MvToPointType : size_t {
    SELECT_INDEX_POINT,
    MVTO_REQUST_WRITE_POINT,
    MVTO_REQUST_READ_POINT,
    MVTO_WORKSET_POINT
};
