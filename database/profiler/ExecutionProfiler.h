...
#define END_TRANSACTION_TIME_MEASURE(thread_id, txn_type)        \
    ...
    if (execution_stat_[thread_id].find(txn_type) ==             \
        execution_stat_[thread_id].end()) {                      \
        execution_stat_[thread_id][txn_type] =                   \
            execution_timer_[thread_id].GetElapsedNanoSeconds(); \
    } else {                                                     \
        execution_stat_[thread_id][txn_type] +=                  \
            execution_timer_[thread_id].GetElapsedNanoSeconds(); \
    }
...
#define REPORT_EXECUTION_PROFILER                                             \
    ...
    for (int i = 0; i < gThreadCount; ++i) {                                  \
        std::map<size_t, long long> ordered_stat(execution_stat_[i].begin(),  \
                                                 execution_stat_[i].end());   \
