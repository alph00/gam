...
#define INIT_FLOW_EXECUTION_TIME_PROFILER                                      \
    core_busy_time_stat_ = new long long[gThreadCount];                        \
    for (size_t i = 0; i < gThreadCount; ++i) core_busy_time_stat_[i] = 0;     \
    core_wait_time_stat_ = new long long[gThreadCount];                        \
    for (size_t i = 0; i < gThreadCount; ++i) core_wait_time_stat_[i] = 0;     \
    slice_busy_time_stat_ = new long long *[gThreadCount];                     \
    for (size_t i = 0; i < gThreadCount; ++i) {                                \
        slice_busy_time_stat_[i] = new long long[kMaxSliceNum];                \
        memset(slice_busy_time_stat_[i], 0, kMaxSliceNum * sizeof(long long)); \
    }                                                                          \
    slice_wait_time_stat_ = new long long *[gThreadCount];                     \
    for (size_t i = 0; i < gThreadCount; ++i) {                                \
        slice_wait_time_stat_[i] = new long long[kMaxSliceNum];                \
        memset(slice_wait_time_stat_[i], 0, kMaxSliceNum * sizeof(long long)); \
    }                                                                          \
    core_busy_wait_timer_ = new PreciseTimeMeasurer[gThreadCount];             \
    core_execute_timer_ = new PreciseTimeMeasurer[gThreadCount];               \
    core_wait_count_stat_ = new long long[gThreadCount];                       \
    for (size_t i = 0; i < gThreadCount; ++i) core_wait_count_stat_[i] = 0;
#else
#define INIT_FLOW_EXECUTION_TIME_PROFILER                                      \
    core_busy_time_stat_ = new long long[gThreadCount];                        \
    for (size_t i = 0; i < gThreadCount; ++i) core_busy_time_stat_[i] = 0;     \
    ...
    core_busy_wait_timer_ = new TimeMeasurer[gThreadCount];                    \
    core_execute_timer_ = new TimeMeasurer[gThreadCount];                      \
    core_wait_count_stat_ = new long long[gThreadCount];                       \
    for (size_t i = 0; i < gThreadCount; ++i) core_wait_count_stat_[i] = 0;
#endif
...
#define END_CENTRAL_EXECUTION_TIME_MEASURE \
    ...
    central_execution_time_stat_ +=        \
        central_execution_timer_.GetElapsedNanoSeconds();

#define REPORT_FLOW_EXECUTION_TIME_PROFILER(thread_count)                     \
    printf(                                                                   \
        "**********BEGIN BATCH EXECUTION TIME PROFILING REPORT**********\n"); \
    printf("<<<<<<<<<<<<<<<<<<< CORE TIME PROFILE\n");                        \
    for (int i = 0; i < gThreadCount; ++i) {                                  \
        if (core_busy_time_stat_[i] != 0) {                                   \
            printf(                                                           \
                "core_id=%d, busy time=%lld ms, waiting time=%lld ms, "       \
                "waiting count=%lld\n",                                       \
                i, core_busy_time_stat_[i] / 1000 / 1000,                     \
                core_wait_time_stat_[i] / 1000 / 1000,                        \
                core_wait_count_stat_[i]);                                    \
            for (int j = 0; j < kMaxSliceNum; ++j) {                          \
                if (slice_busy_time_stat_[i][j] != 0) {                       \
                    printf("busy slice id=%d, busy slice time=%lld ms\n", j,  \
                           slice_busy_time_stat_[i][j] / 1000 / 1000);        \
                }                                                             \
            }                                                                 \
            for (int j = 0; j < kMaxSliceNum; ++j) {                          \
                if (slice_wait_time_stat_[i][j] != 0) {                       \
                    printf("wait slice id=%d, wait slice time=%lld ms\n", j,  \
                           slice_wait_time_stat_[i][j] / 1000 / 1000);        \
                }                                                             \
            }                                                                 \
        }                                                                     \
    }                                                                         \
    for (int slice_id = 0; slice_id < kMaxSliceNum; ++slice_id) {             \
        long long total = 0;                                                  \
        for (int tid = 0; tid < gThreadCount; ++tid) {                        \
            total += slice_busy_time_stat_[tid][slice_id];                    \
        }                                                                     \
        if (total != 0) {                                                     \
            printf("slice id=%d, total time=%lld ms\n", slice_id,             \
                   total / 1000 / 1000 / thread_count);                       \
        }                                                                     \
    }                                                                         \
    printf("parameter checker elapsed time = %lld ms\n",                      \
           central_execution_time_stat_ / 1000 / 1000);                       \
    printf("**********END BATCH EXECUTION TIME PROFILING "                    \
           "REPORT**********\n\n");
...
#define END_CORE_BUSY_TIME_MEASURE(core_id, slice_id)           \
    ...
    long long busy_elapsed_time =                               \
        core_busy_wait_timer_[core_id].GetElapsedNanoSeconds(); \
...
#define INC_CORE_WAIT_COUNT(core_id) ++core_wait_count_stat_[core_id];
#else
#define INIT_FLOW_EXECUTION_TIME_PROFILER ;
#define REPORT_FLOW_EXECUTION_TIME_PROFILER(thread_count) ;
