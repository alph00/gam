...
if (gam_op_stat_[thread_id].find(gam_operation_type) == gam_op_stat_[thread_id].end()){ \
	gam_op_stat_[thread_id][gam_operation_type] = gam_op_timer_[thread_id].GetElapsedNanoSeconds(); \
} \
else{ \
	gam_op_stat_[thread_id][gam_operation_type] += gam_op_timer_[thread_id].GetElapsedNanoSeconds(); \
}
...
#define REPORT_GAM_OPERATION_PROFILER \
	printf("**********BEGIN GAM_OPERATION PROFILING REPORT(NOT INCLUDE INDEX RELATED OPERATIONS)**********\n"); \
	std::map<size_t, long long> gam_op_time_total; \
for (int i = 0; i < gThreadCount; ++i){ \
	std::map<size_t, long long> ordered_stat(gam_op_stat_[i].begin(), gam_op_stat_[i].end()); \
for (auto &entry : ordered_stat){ \
	gam_op_time_total[entry.first] += entry.second / 1000 / 1000; \
	printf("thread_id = %d, gam operation = %s, elapsed_time = %lld ms\n", i, gam_operation_type_string[(int)entry.first].c_str(), entry.second / 1000 / 1000); \
} \
} \
	printf("\n"); \
	for (auto& entry : gam_op_time_total) { \
		printf("gam operation = %s, avg elapsed_time = %lld ms\n", gam_operation_type_string[(int)entry.first].c_str(), entry.second / gThreadCount); \
	} \
...
namespace Database{
	...
	const static std::string gam_operation_type_string[kGamOpCount] = { "GAM_READ",
			"GAM_WRITE", "GAM_RLOCK", "GAM_WLOCK",
			"GAM_TRY_RLOCK", "GAM_TRY_WLOCK", "GAM_UNLOCK", "GAM_MALLOC", "GAM_FREE" };
