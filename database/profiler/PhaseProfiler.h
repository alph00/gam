...
for (auto &entry : ordered_stat){ \
	...
	printf("thread_id = %d, phase = %s, elapsed_time = %lld ms\n", i, phase_type_string[(int)entry.first].c_str(), entry.second / 1000 / 1000); \
} \
} \
	printf("\n"); \
	for (auto& entry : phase_time_total) { \
		printf("phase = %s, avg elapsed_time = %lld ms\n", phase_type_string[(int)entry.first].c_str(), entry.second / gThreadCount); \
	} \
...
namespace Database{
	...
	enum PhaseType
		...
		TXN_ABORT,
		TXN_EXECUTE,
		LOCK_READ_TEST,  // NOTE(weihaosun): add "TEST" suffix since "LOCK_READ" is a natively already defined macro 
