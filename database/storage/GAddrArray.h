...
class GAddrArray : public GAMObject {
   public:
    ...
    void Init(const uint64_t& element_count, size_t element_size,
              GAlloc* gallocator) {
            ...
            for (int layer_id = 1; layer_id < layer_num_; ++layer_id) {
                for (uint64_t pos = 0; pos < layer_size_[layer_id]; ++pos) {
                    size_t per_slot_size = layer_id < layer_num_ - 1
                                               ...
                                               : element_size;
                    GAddr mem_segment =
                        gallocator->AlignedMalloc(per_slot_size * N);
                    GAddr parent_addr =
                        LocateParentNode(layer_id, pos, gallocator);
   ...
   private:
    ...
    GAddr LocateParentNode(int layer_id, uint64_t pos, GAlloc* gallocator) {
        ...
        for (int j = 1; j <= layer_id - 1; ++j) {
            GAddr tmp = 0;
            gallocator->Read(cur_addr, path[j] * sizeof(GAddr), &tmp,
                             sizeof(GAddr));
            cur_addr = tmp;
        }
        return cur_addr;
