...
struct ItemsIter {
    ItemsIter(const GAddr& items_addr) : items_addr_(items_addr), pos_(-1) {}
...
class BucketHeader : public GAMObject {
   public:
    ...
    bool InsertNode(const IndexKey& key, const GAddr& record_ptr,
        ...
        if (cur_node_addr != Gnullptr) {  // There exists a match for key
            ...
            if (items.AddRecordPtr(record_ptr)) {  // success
                ...
                gallocator->Write(new_items_addr, &new_items,
                                  sizeof(new_items));
            ...
            if (prev_node_addr != Gnullptr) {
                ...
                gallocator->Write(prev_node_addr, &prev_node,
                                  sizeof(BucketNode));
    ...
    static size_t GetSerializeSize() { return sizeof(GAddr); }
