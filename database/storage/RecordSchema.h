...
class RecordSchema : public GAMObject {
   public:
    RecordSchema(size_t table_id) : table_id_(table_id) {
        column_count_ = 0;
        column_offset_ = 0;
    ...
    void AddSecondaryColumns(const size_t* column_ids,
    ...
    void SetPartitionColumns(const size_t* column_ids,
                             const size_t& column_num) {
        assert(column_num < 5);
    ...
    const size_t& GetTableId() const { return table_id_; }
    const size_t& GetSecondaryCount() const { return secondary_index_num_; }
    const size_t& GetPartitionCount() const { return partition_column_num_; }
    const size_t GetMetaColumnId() const { return column_count_ - 1; }
    ...
    virtual void Serialize(const GAddr& addr, GAlloc* gallocator) {
        ...
        for (size_t i = 0; i < 3; ++i) {
            gallocator->Write(addr, off, secondary_column_ids_[i],
                              sizeof(size_t) * 5);
            off += sizeof(size_t) * 5;
        }
        ...
        off += sizeof(size_t);
        gallocator->Write(addr, off, columns_,
                          sizeof(ColumnInfo) * column_count_);
    }
    ...
    virtual void Deserialize(const GAddr& addr, GAlloc* gallocator) {
        ...
        off += sizeof(size_t);
        for (size_t i = 0; i < 3; ++i) {
            gallocator->Read(addr, off, secondary_column_ids_[i],
                             sizeof(size_t) * 5);
            off += sizeof(size_t) * 5;
        }
        ...
        off += sizeof(size_t);
        columns_ = new ColumnInfo[column_count_];
        gallocator->Read(addr, off, columns_,
                         sizeof(ColumnInfo) * column_count_);
    }
