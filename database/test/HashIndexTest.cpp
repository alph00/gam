...
void ThreadExecute(size_t partition_id, size_t thread_id,
    ...
    for (int i = 0; i < num_txn; ++i) {
        ...
        bool ret =
            hash_index->InsertRecord(key, data_addr, gallocator, thread_id);
        assert(ret);
    }
}
void ThreadCheck(size_t partition_id, size_t thread_id, HashIndex *hash_index) {
    ...
    for (int i = 0; i < num_txn; ++i) {
        ...
        GAddr data_addr = hash_index->SearchRecord(key, gallocator, thread_id);
...
int main(int argc, char *argv[]) {
    ...
    if (config.IsMaster()) {
        storage_addr =
            default_gallocator->AlignedMalloc(HashIndex::GetSerializeSize());
        HashIndex hash_index;
...

    {
        ...
        for (size_t i = 0; i < gThreadCount; ++i) {
            thread_group.create_thread(boost::bind(
                &ThreadExecute, config.GetMyPartitionId(), i, &hash_index));
        }
        is_begin = true;
    ...
    std::cout << "start check..." << std::endl;
    {
        ...
        for (size_t i = 0; i < gThreadCount; ++i) {
            thread_group.create_thread(boost::bind(
                &ThreadCheck, config.GetMyPartitionId(), i, &hash_index));
        }
        is_begin = true;
        thread_group.join_all();
    }
