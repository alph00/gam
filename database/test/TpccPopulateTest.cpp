...
void CheckSchema(RecordSchema *schema) {
    ...
    switch (schema->GetTableId()) {
        ...
        case DISTRICT_NEW_ORDER_TABLE_ID:
            TpccInitiator::InitDistrictNewOrderSchema(expect,
                                                      default_gallocator);
            break;
...
void Check(GAddr storage_addr) {
    ...
    for (int i_id = 1; i_id <= tpcc_scale_params.num_items_; ++i_id) {
        for (int w_id = tpcc_scale_params.starting_warehouse_;
            ...
            assert(ret);
            Record *item_record =
                new Record(storage_manager.tables_[ITEM_TABLE_ID]->GetSchema());
    ...
    for (int w_id = 1; w_id <= tpcc_scale_params.num_warehouses_; ++w_id) {
        ...
        for (int d_id = 1;
             d_id <= tpcc_scale_params.num_districts_per_warehouse_; ++d_id) {
            ...
            assert(act_d_next_o_id ==
                   tpcc_scale_params.num_customers_per_district_ + 1);
            delete district_record;
            district_record = nullptr;

            for (int c_id = 1;
                 c_id <= tpcc_scale_params.num_customers_per_district_;
                 ++c_id) {
            ...
            ret = storage_manager.tables_[DISTRICT_NEW_ORDER_TABLE_ID]
                      ->SearchRecord(GetDistrictNewOrderPrimaryKey(d_id, w_id),
                                     gallocators[0], 0);
            assert(ret);
            Record *district_new_order_record =
                new Record(storage_manager.tables_[DISTRICT_NEW_ORDER_TABLE_ID]
                               ->GetSchema());
            ...
            int initial_new_order_id =
                ...
                tpcc_scale_params.num_new_orders_per_district_ + 1;
            for (int o_id = 1;
                 o_id <= tpcc_scale_params.num_customers_per_district_;
                 ++o_id) {
                ...
                assert(act_carrier_id == NULL_CARRIER_ID ||
                       (MIN_CARRIER_ID <= act_carrier_id &&
                        act_carrier_id <= MAX_CARRIER_ID));
                int act_all_data = -1;
                ...
                if (o_id >= initial_new_order_id) {
                    ret = storage_manager.tables_[NEW_ORDER_TABLE_ID]
                              ->SearchRecord(
                                  GetNewOrderPrimaryKey(o_id, d_id, w_id),
                                  gallocators[0], 0);
                    assert(ret);
                    Record *new_order_record =
                        new Record(storage_manager.tables_[NEW_ORDER_TABLE_ID]
                                       ->GetSchema());
