...
class TpccPopulator : public BenchmarkPopulator {
   public:
    TpccPopulator(StorageManager *storage_manager,
        ...
        : BenchmarkPopulator(storage_manager), scale_params_(scale_params) {}
    ...
    virtual void StartPopulate() {
        ...
        Record *item_record_buf =
            new Record(storage_manager_->tables_[ITEM_TABLE_ID]->GetSchema());
        ...
        Record *district_new_order_record_buf =
            new Record(storage_manager_->tables_[DISTRICT_NEW_ORDER_TABLE_ID]
                           ->GetSchema());
        Record *order_record_buf =
            new Record(storage_manager_->tables_[ORDER_TABLE_ID]->GetSchema());
        ...
        Record *stock_record_buf =
            new Record(storage_manager_->tables_[STOCK_TABLE_ID]->GetSchema());

        TableRecord *item_table_record_buf = new TableRecord(item_record_buf);
        TableRecord *warehouse_table_record_buf =
            new TableRecord(warehouse_record_buf);
        TableRecord *district_table_record_buf =
            new TableRecord(district_record_buf);
        TableRecord *customer_table_record_buf =
            new TableRecord(customer_record_buf);
        TableRecord *history_table_record_buf =
            new TableRecord(history_record_buf);
        TableRecord *district_new_order_table_record_buf =
            new TableRecord(district_new_order_record_buf);
        TableRecord *order_table_record_buf = new TableRecord(order_record_buf);
        TableRecord *new_order_table_record_buf =
            new TableRecord(new_order_record_buf);
        TableRecord *order_line_table_record_buf =
            new TableRecord(order_line_record_buf);
        ...
        for (int item_id = 1; item_id <= scale_params_->num_items_; ++item_id) {
            bool is_origin =
                (original_rows.find(item_id) != original_rows.end());
        ...
        for (int w_id = scale_params_->starting_warehouse_;
            ...
            InsertWarehouseRecord(warehouse_record, warehouse_table_record_buf,
                                  0);
            for (int d_id = 1;
                 d_id <= scale_params_->num_districts_per_warehouse_; ++d_id) {
                int d_next_o_id =
                    scale_params_->num_customers_per_district_ + 1;
                // generate districts
                GenerateDistrictRecord(w_id, d_id, d_next_o_id,
                                       district_record);
                InsertDistrictRecord(district_record, district_table_record_buf,
                                     0);
                ...
                for (int o_id = 1;
                    ...
                    for (int ol_number = 1; ol_number <= o_ol_cnt;
                        ...
                        InsertOrderLineRecord(order_line_record,
                                              order_line_table_record_buf, 0);
                    }

                    if (is_new_order) {
                        // generate new orders
                        GenerateNewOrderRecord(w_id, d_id, o_id,
                                               new_order_record);
                        InsertNewOrderRecord(new_order_record,
                                             new_order_table_record_buf, 0);
                    }
                }
            }
            std::unordered_set<int> selected_rows;
            TpccRandomGenerator::SelectUniqueIds(scale_params_->num_items_ / 10,
                                                 1, scale_params_->num_items_,
                                                 selected_rows);
            for (int i_id = 1; i_id <= scale_params_->num_items_; ++i_id) {
                bool original =
                    (selected_rows.find(i_id) != selected_rows.end());
                // generate stocks
   ...
   private:
    ...
    void GenerateItemRecord(const int &item_id, const int &w_id, bool original,
        ...
        std::string name =
            TpccRandomGenerator::GenerateAString(MIN_I_NAME, MAX_I_NAME);
        memcpy(record->i_name_, name.c_str(), name.size());
        record->i_price_ = TpccRandomGenerator::GenerateFixedPoint(
            MONEY_DECIMALS, MIN_PRICE, MAX_PRICE);
        std::string data =
            TpccRandomGenerator::GenerateAString(MIN_I_DATA, MAX_I_DATA);
        memcpy(record->i_data_, data.c_str(), data.size());
    }

    void GenerateWarehouseRecord(const int &w_id,
                                 WarehouseRecord *record) const {
        record->w_id_ = w_id;
        std::string name =
            TpccRandomGenerator::GenerateAString(MIN_NAME, MAX_NAME);
        memcpy(record->w_name_, name.c_str(), name.size());
        TpccRandomGenerator::GenerateAddress(
            record->w_street_1_, record->w_street_2_, record->w_city_,
    ...
    void GenerateDistrictRecord(const int &d_w_id, const int &d_id,
        ...
        TpccRandomGenerator::GenerateAddress(
            record->d_street_1_, record->d_street_2_, record->d_city_,
    ...
    void GenerateCustomerRecord(const int &c_w_id, const int &c_d_id,
        ...
        std::string first =
            TpccRandomGenerator::GenerateAString(MIN_FIRST, MAX_FIRST);
        ...
        TpccRandomGenerator::GenerateAddress(
            record->c_street_1_, record->c_street_2_, record->c_city_,
    ...
    void GenerateStockRecord(const int &s_w_id, const int &s_i_id,
                             bool original, StockRecord *record) const {
        record->s_i_id_ = s_i_id;
        record->s_w_id_ = s_w_id;
        record->s_quantity_ =
            TpccRandomGenerator::GenerateInteger(MIN_QUANTITY, MAX_QUANTITY);
    ...
    void GenerateNewOrderRecord(const int &w_id, const int &d_id,
                                const int &o_id, NewOrderRecord *record) const {
        record->w_id_ = w_id;
        record->d_id_ = d_id;
        record->o_id_ = o_id;
    }
    ...
    void GenerateOrderLineRecord(const int &ol_w_id, const int &ol_d_id,
        ...
        std::string ol_dist_info =
            TpccRandomGenerator::GenerateAString(DIST, DIST);
        memcpy(record->ol_dist_info_, ol_dist_info.c_str(),
               ol_dist_info.size());
    }
    ...
    void GenerateHistoryRecord(const int &h_c_w_id, const int &h_c_d_id,
        ...
        std::string data =
            TpccRandomGenerator::GenerateAString(MIN_DATA, MAX_DATA);
    ...
    void InsertWarehouseRecord(WarehouseRecord *record_ptr,
                               TableRecord *table_record_buf,
                               size_t thread_id) {
        GAlloc *gallocator = gallocators[thread_id];
        Record *record_buf = table_record_buf->record_;
        GAddr data_addr =
    ...
    void InsertDistrictRecord(DistrictRecord *record_ptr,
        ...
        record_buf->Serialize(data_addr, gallocator);
        IndexKey k =
            GetDistrictPrimaryKey(record_ptr->d_id_, record_ptr->d_w_id_);
        storage_manager_->tables_[DISTRICT_TABLE_ID]->InsertRecord(
            &k, 1, data_addr, gallocator, thread_id);
    }

    void InsertCustomerRecord(CustomerRecord *record_ptr,
                              TableRecord *table_record_buf, size_t thread_id) {
        GAlloc *gallocator = gallocators[thread_id];
        Record *record_buf = table_record_buf->record_;
        GAddr data_addr =
        ...
        IndexKey key = GetCustomerPrimaryKey(
            record_ptr->c_id_, record_ptr->c_d_id_, record_ptr->c_w_id_);
        storage_manager_->tables_[CUSTOMER_TABLE_ID]->InsertRecord(
            &key, 1, data_addr, gallocator, thread_id);
    }

    void InsertStockRecord(StockRecord *record_ptr,
                           TableRecord *table_record_buf, size_t thread_id) {
        GAlloc *gallocator = gallocators[thread_id];
        Record *record_buf = table_record_buf->record_;
        GAddr data_addr =
        ...
        record_buf->Serialize(data_addr, gallocator);
        IndexKey key =
            GetStockPrimaryKey(record_ptr->s_i_id_, record_ptr->s_w_id_);
        storage_manager_->tables_[STOCK_TABLE_ID]->InsertRecord(
            &key, 1, data_addr, gallocator, thread_id);
    }

    void InsertOrderRecord(OrderRecord *record_ptr,
                           TableRecord *table_record_buf, size_t thread_id) {
        GAlloc *gallocator = gallocators[thread_id];
        Record *record_buf = table_record_buf->record_;
        GAddr data_addr =
        ...
        record_buf->SetVisible(true);
        record_buf->Serialize(data_addr, gallocator);
        IndexKey key = GetOrderPrimaryKey(
            record_ptr->o_id_, record_ptr->o_d_id_, record_ptr->o_w_id_);
        storage_manager_->tables_[ORDER_TABLE_ID]->InsertRecord(
            &key, 1, data_addr, gallocator, thread_id);
    }

    void InsertNewOrderRecord(NewOrderRecord *record_ptr,
                              TableRecord *table_record_buf, size_t thread_id) {
        GAlloc *gallocator = gallocators[thread_id];
        Record *record_buf = table_record_buf->record_;
        GAddr data_addr =
        ...
        record_buf->SetVisible(true);
        record_buf->Serialize(data_addr, gallocator);
        IndexKey key = GetNewOrderPrimaryKey(
            record_ptr->o_id_, record_ptr->d_id_, record_ptr->w_id_);
        storage_manager_->tables_[NEW_ORDER_TABLE_ID]->InsertRecord(
            &key, 1, data_addr, gallocator, thread_id);
    }

    void InsertOrderLineRecord(OrderLineRecord *record_ptr,
                               TableRecord *table_record_buf,
                               size_t thread_id) {
        GAlloc *gallocator = gallocators[thread_id];
        Record *record_buf = table_record_buf->record_;
        ...
        record_buf->SetVisible(true);
        record_buf->Serialize(data_addr, gallocator);
        IndexKey key = GetOrderLinePrimaryKey(
            record_ptr->ol_o_id_, record_ptr->ol_d_id_, record_ptr->ol_w_id_,
            record_ptr->ol_number_);
        // keys[1] = GetOrderLineSecondaryKey(record_ptr->ol_o_id_,
        // record_ptr->ol_d_id_, record_ptr->ol_w_id_);
        storage_manager_->tables_[ORDER_LINE_TABLE_ID]->InsertRecord(
            &key, 1, data_addr, gallocator, thread_id);
    }

    void InsertHistoryRecord(HistoryRecord *record_ptr,
                             TableRecord *table_record_buf, size_t thread_id) {
        GAlloc *gallocator = gallocators[thread_id];
        Record *record_buf = table_record_buf->record_;
        GAddr data_addr =
        ...
        record_buf->SetVisible(true);
        record_buf->Serialize(data_addr, gallocator);
        IndexKey key = GetHistoryPrimaryKey(
            record_ptr->h_c_id_, record_ptr->h_d_id_, record_ptr->h_w_id_);
    ...
    void InsertDistrictNewOrderRecord(DistrictNewOrderRecord *record_ptr,
        ...
        record_buf->Serialize(data_addr, gallocator);
        IndexKey key =
            GetDistrictNewOrderPrimaryKey(record_ptr->d_id_, record_ptr->w_id_);
