...
class DeliveryProcedure : public StoredProcedure {
   public:
    DeliveryProcedure() { context_.txn_type_ = DELIVERY; }
    ...
    virtual bool Execute(TxnParam *param, CharArray &ret) {
        ...
        for (int no_d_id = 1; no_d_id <= DISTRICTS_PER_WAREHOUSE; ++no_d_id) {
            /*SELECT NO_O_ID FROM " + TPCCConstants.TABLENAME_NEWORDER + " WHERE
             NO_D_ID = ? AND NO_W_ID = ? ORDER BY NO_O_ID ASC LIMIT 1 DELETE
             FROM " + TPCCConstants.TABLENAME_NEWORDER WHERE NO_O_ID = ? AND
             NO_D_ID = ? we workaround here to avoid 'order by', 'limit',
             'delete', by introducing a new table DISTRICT_NEW_ORDER, get
             no_o_id and update it*/
            Record *district_new_order_record = nullptr;
            IndexKey district_new_order_key =
                GetDistrictNewOrderPrimaryKey(no_d_id, delivery_param->w_id_);
            DB_QUERY(SearchRecord(&context_, DISTRICT_NEW_ORDER_TABLE_ID,
                                  district_new_order_key,
                                  district_new_order_record, READ_WRITE));
            int no_o_id = 0;
            district_new_order_record->GetColumn(2, &no_o_id);
            assert(no_o_id != 0);

            IndexKey new_order_key =
                GetNewOrderPrimaryKey(no_o_id, no_d_id, delivery_param->w_id_);
            Record *new_order_record = nullptr;
            if (transaction_manager_->SearchRecord(
                    &context_, NEW_ORDER_TABLE_ID, new_order_key,
                    new_order_record, READ_ONLY)) {
        ...
        for (int no_d_id = 1; no_d_id <= DISTRICTS_PER_WAREHOUSE; ++no_d_id) {
            ...
            // SELECT O_C_ID  FROM TPCCConstants.TABLENAME_OPENORDER WHERE O_ID
            // = ? AND O_D_ID = ? AND O_W_ID = ? UPDATE " +
            // TPCCConstants.TABLENAME_OPENORDER + " SET O_CARRIER_ID = ? WHERE
            // O_ID = ? AND O_D_ID = ? AND O_W_ID = ?
            IndexKey order_key = GetOrderPrimaryKey(
                no_o_ids[no_d_id - 1], no_d_id, delivery_param->w_id_);
            Record *order_record = nullptr;
            DB_QUERY(SearchRecord(&context_, ORDER_TABLE_ID, order_key,
                                  order_record, READ_WRITE));
        ...
        // 	IndexKey order_line_key =
        // GetOrderLineSecondaryKey(no_o_ids[no_d_id - 1], no_d_id,
        // delivery_param->w_id_); 	Records order_line_records(15);
        // 	// "updateOrderLine": "UPDATE ORDER_LINE SET OL_DELIVERY_D = ?
        // WHERE OL_O_ID = ? AND OL_D_ID = ? AND OL_W_ID = ?"
        // 	// "sumOLAmount": "SELECT SUM(OL_AMOUNT) FROM ORDER_LINE WHERE
        // OL_O_ID = ? AND OL_D_ID = ? AND OL_W_ID = ?"
        // 	// access secondary index here !
        // 	DB_QUERY(SearchRecords(&context_, ORDER_LINE_TABLE_ID, 1,
        // order_line_key, &order_line_records, READ_WRITE));

        // 	double sum = 0, tmp = 0;
        // 	ItemsIter iter(order_line_items_addr);
        // 	GAddr record_addr = 0;
        // 	while ((record_addr = iter.GetNext(gallocators[thread_id_])) !=
        // 0){ 		RecordGAM order_line_record;
        // 		gallocators[thread_id_]->Read(record_addr,
        // &order_line_record, sizeof(RecordGAM));
        // 		order_line_record.SetColumn(6, order_line_schema_ptr_,
        // &delivery_param->ol_delivery_d_, gallocators[thread_id_]);
        // 		order_line_record.GetColumn(8, order_line_schema_ptr_,
        // &tmp, gallocators[thread_id_]); 		sum += tmp;
        // 	}
        // 	sums[no_d_id - 1] = sum;
        // }
        ...
        for (int no_d_id = 1; no_d_id <= DISTRICTS_PER_WAREHOUSE; ++no_d_id) {
            ...
            IndexKey customer_key = GetCustomerPrimaryKey(
                c_ids[no_d_id - 1], no_d_id, delivery_param->w_id_);
            Record *customer_record = nullptr;
            DB_QUERY(SearchRecord(&context_, CUSTOMER_TABLE_ID, customer_key,
                                  customer_record, READ_WRITE));
            double balance = 0.0;
        ...
        for (size_t no_d_id = 1; no_d_id <= DISTRICTS_PER_WAREHOUSE;
             ++no_d_id) {
            ret.Memcpy(ret.size_, (char *)(&no_o_ids[no_d_id - 1]),
                       sizeof(int));
...
class NewOrderProcedure : public StoredProcedure {
   public:
    NewOrderProcedure() { context_.txn_type_ = NEW_ORDER; }
    ...
    virtual bool Execute(TxnParam *param, CharArray &ret) {
        ...
        for (size_t i = 0; i < new_order_param->ol_cnt_; ++i) {
            ...
            // "getItemInfo": "SELECT I_PRICE, I_NAME, I_DATA FROM ITEM WHERE
            // I_ID = ?"
            IndexKey item_key =
                GetItemPrimaryKey(item_id, new_order_param->w_id_);
            Record *item_record = nullptr;
            if (transaction_manager_->SearchRecord(
                    &context_, ITEM_TABLE_ID, item_key, item_record,
                    (AccessType)new_order_param->item_access_type_[i]) ==
                false) {
                // currently disable application-level abort, impossible to
                // return false
                assert(false);
        ...
        for (size_t i = 0; i < new_order_param->ol_cnt_; ++i) {
            ...
            // = ?" "updateStock": "UPDATE STOCK SET S_QUANTITY = ?, S_YTD = ?,
            ...
            // DB_QUERY(SearchRecord(&context_, STOCK_TABLE_ID, stock_key,
            // stock_record, READ_WRITE));
            DB_QUERY(SearchRecord(
                &context_, STOCK_TABLE_ID, stock_key, stock_record,
                (AccessType)
                    new_order_param->stock_access_type_[i]));  // for testing
        ...
        DB_QUERY(SearchRecord(
            &context_, WAREHOUSE_TABLE_ID, warehouse_key, warehouse_record,
            (AccessType)new_order_param->warehouse_access_type_));
        double w_tax = 0;
        warehouse_record->GetColumn(7, &w_tax);

        // "getDistrict": "SELECT D_TAX, D_NEXT_O_ID FROM DISTRICT WHERE D_ID =
        // ? AND D_W_ID = ?" "incrementNextOrderId": "UPDATE DISTRICT SET
        // D_NEXT_O_ID = ? WHERE D_ID = ? AND D_W_ID = ?"
        IndexKey district_key = GetDistrictPrimaryKey(new_order_param->d_id_,
                                                      new_order_param->w_id_);
        Record *district_record = nullptr;
        DB_QUERY(SearchRecord(
            &context_, DISTRICT_TABLE_ID, district_key, district_record,
        ...
        Record *customer_record = nullptr;
        DB_QUERY(SearchRecord(
            &context_, CUSTOMER_TABLE_ID, customer_key, customer_record,
        ...
        // VALUES (?, ?, ?)" BEGIN_GAM_OPERATION_MEASURE(thread_id_,
        // GAM_MALLOC); GAddr new_order_addr = gallocators[thread_id_]->Malloc(
        //     transaction_manager_->storage_manager_->
        //     tables_[NEW_ORDER_TABLE_ID]->GetSchemaSize());
        // NOTE(weihaosun): for concurrency control, we need to store
        // table_record rather than raw record NOTE(weihaosun): for general cc
        // benchmarks, insert is omitted. Thus, we move Gam Malloc to Commit
        // Phase GAddr new_order_addr = gallocators[thread_id_]->Malloc(
        ...
        //   // new_order_record->Serialize(new_order_addr,
        ...
        // //   END_GAM_OPERATION_MEASURE(thread_id_, GAM_WRITE);
        // // }
        // IndexKey order_key = GetOrderPrimaryKey(d_next_o_id,
        // new_order_param->d_id_,
        //                                         new_order_param->w_id_);
        // DB_QUERY(
        ...
        //   int ol_supply_w_id = new_order_param->i_w_ids_[i];
        //   int ol_quantity = new_order_param->i_qtys_[i];
        //   // "createOrderLine": "INSERT INTO ORDER_LINE (OL_O_ID, OL_D_ID,
        //   OL_W_ID, OL_NUMBER, OL_I_ID, OL_SUPPLY_W_ID, OL_DELIVERY_D,
        ...
        //   order_line_record->SetColumn(6, (char*)
        //   (&new_order_param->o_entry_d_)); order_line_record->SetColumn(7,
        //   (char*) (&ol_quantity)); order_line_record->SetColumn(8, (char*)
        //   (&ol_amounts[i])); order_line_record->SetColumn(9, s_dists[i]);
        //   order_line_record->SetVisible(true);
        //   // if (new_order_param->order_line_access_type_[i] != READ_ONLY) {
        //   //   BEGIN_GAM_OPERATION_MEASURE(thread_id_, GAM_WRITE);
        //   //   order_line_record->Serialize(order_line_addr,
        //   gallocators[thread_id_]);
        //   //   END_GAM_OPERATION_MEASURE(thread_id_, GAM_WRITE);
        //   // }
        ...
        //                                                    new_order_param->w_id_,
        //                                                    ol_number);
        //   //order_line_keys[1] = GetOrderLineSecondaryKey(d_next_o_id,
        //   new_order_param->d_id_, new_order_param->w_id_); DB_QUERY(
...
class PaymentProcedure : public StoredProcedure {
   public:
    PaymentProcedure() { context_.txn_type_ = PAYMENT; }
    ...
    virtual bool Execute(TxnParam *param, CharArray &ret) {
        ...
        // "getWarehouse": "SELECT W_NAME, W_STREET_1, W_STREET_2, W_CITY,
        // W_STATE, W_ZIP FROM WAREHOUSE WHERE W_ID = ?"
        // "updateWarehouseBalance": "UPDATE WAREHOUSE SET W_YTD = W_YTD + ?
        // WHERE W_ID = ?"
        IndexKey warehouse_key = GetWarehousePrimaryKey(payment_param->w_id_);
        Record *warehouse_record = nullptr;
        DB_QUERY(SearchRecord(
            &context_, WAREHOUSE_TABLE_ID, warehouse_key, warehouse_record,
            (AccessType)payment_param->warehouse_access_type_));
        double w_ytd = 0;
        ...
        // "getDistrict": "SELECT D_NAME, D_STREET_1, D_STREET_2, D_CITY,
        // D_STATE, D_ZIP FROM DISTRICT WHERE D_W_ID = ? AND D_ID = ?"
        // "updateDistrictBalance": "UPDATE DISTRICT SET D_YTD = D_YTD + ? WHERE
        // D_W_ID  = ? AND D_ID = ?"
        IndexKey district_key =
            GetDistrictPrimaryKey(payment_param->d_id_, payment_param->w_id_);
        Record *district_record = nullptr;
        DB_QUERY(SearchRecord(
            &context_, DISTRICT_TABLE_ID, district_key, district_record,
            (AccessType)payment_param->district_access_type_));
        double d_ytd = 0;
        ...
        if (payment_param->c_id_ == -1) {
            // "getCustomersByLastName": "SELECT C_ID, C_FIRST, C_MIDDLE,
            // C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE,
            // C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE,
            // C_YTD_PAYMENT, C_PAYMENT_CNT, C_DATA FROM CUSTOMER WHERE C_W_ID =
            // ? AND C_D_ID = ? AND C_LAST = ? ORDER BY C_FIRST"

        } else {
            // "getCustomerByCustomerId": "SELECT C_ID, C_FIRST, C_MIDDLE,
            // C_LAST, C_STREET_1, C_STREET_2, C_CITY, C_STATE, C_ZIP, C_PHONE,
            // C_SINCE, C_CREDIT, C_CREDIT_LIM, C_DISCOUNT, C_BALANCE,
            // C_YTD_PAYMENT, C_PAYMENT_CNT, C_DATA FROM CUSTOMER WHERE C_W_ID =
            // ? AND C_D_ID = ? AND C_ID = ?"
            IndexKey customer_key = GetCustomerPrimaryKey(
                payment_param->c_id_, payment_param->c_d_id_,
                payment_param->c_w_id_);
            DB_QUERY(SearchRecord(
                &context_, CUSTOMER_TABLE_ID, customer_key, customer_record,
                (AccessType)payment_param->customer_access_type_));
        }
        // "updateBCCustomer": "UPDATE CUSTOMER SET C_BALANCE = ?, C_YTD_PAYMENT
        // = ?, C_PAYMENT_CNT = ?, C_DATA = ? WHERE C_W_ID = ? AND C_D_ID = ?
        // AND C_ID = ?" "updateGCCustomer": "UPDATE CUSTOMER SET C_BALANCE = ?,
        // C_YTD_PAYMENT = ?, C_PAYMENT_CNT = ? WHERE C_W_ID = ? AND C_D_ID = ?
        // AND C_ID = ?"
        double balance = 0.0;
        customer_record->GetColumn(16, &balance);
        ...
        // //   BEGIN_GAM_OPERATION_MEASURE(thread_id_, GAM_WRITE);
        // //   history_record->Serialize(history_addr,
        // gallocators[thread_id_]);
        // //   END_GAM_OPERATION_MEASURE(thread_id_, GAM_WRITE);
        // // }
...
class OrderStatusProcedure : public StoredProcedure {
   public:
    OrderStatusProcedure() { context_.txn_type_ = ORDER_STATUS; }
    virtual ~OrderStatusProcedure() {}

    virtual bool Execute(TxnParam *param, CharArray &ret) {
        OrderStatusParam *order_status_param =
            static_cast<OrderStatusParam *>(param);
        ...
        //LIMIT 1"
        // IndexKey order_key = GetOrderPrimaryKey(order_status_param->c_id_,
        // order_status_param->d_id_, order_status_param->w_id_); GAddr
        // order_addr = 0; DB_QUERY(SearchRecord(&context_, ORDER_TABLE_ID,
        // order_key, order_addr, READ_ONLY)); assert(order_addr != 0);
        // RecordGAM order_record;
        // gallocators[thread_id_]->Read(order_addr, &order_record,
        // sizeof(RecordGAM));
        // //"getOrderLines": "SELECT OL_SUPPLY_W_ID, OL_I_ID, OL_QUANTITY,
        // OL_AMOUNT, OL_DELIVERY_D FROM ORDER_LINE WHERE OL_W_ID = ? AND
        // OL_D_ID = ? AND OL_O_ID = ?" int o_id = 0; order_record.GetColumn(0,
        // order_schema_ptr_, &o_id, gallocators[thread_id_]); assert(o_id !=
        // 0); IndexKey order_line_key = GetOrderLineSecondaryKey(o_id,
        // order_status_param->d_id_, order_status_param->w_id_); GAddr
        // order_line_items_addr = 0; DB_QUERY(SearchRecords(&context_,
        // ORDER_LINE_TABLE_ID, 1, order_line_key, order_line_items_addr,
        // READ_ONLY)); assert(order_line_items_addr != 0);

        // ItemsIter iter(order_line_items_addr);
        // GAddr record_addr = 0;
        // while ((record_addr = iter.GetNext(gallocators[thread_id_])) != 0){
        // 	RecordGAM order_line_record;
        // 	gallocators[thread_id_]->Read(record_addr, &order_line_record,
        // sizeof(RecordGAM)); 	int i_id = 0; 	order_line_record.GetColumn(4,
        // order_schema_ptr_, &i_id, gallocators[thread_id_]); 	assert(i_id !=
        // 0); 	ret.Memcpy(ret.size_, (char*)(&i_id), sizeof(i_id)); 	ret.size_ +=
        // sizeof(i_id);
        // }
...
class StockLevelProcedure : public StoredProcedure {
   public:
    StockLevelProcedure() { context_.txn_type_ = STOCK_LEVEL; }
    virtual ~StockLevelProcedure() {}

    virtual bool Execute(TxnParam *param, CharArray &ret) {
        StockLevelParam *stock_level_param =
            static_cast<StockLevelParam *>(param);
        // "getOId": "SELECT D_NEXT_O_ID FROM DISTRICT WHERE D_W_ID = ? AND D_ID
        // = ?" IndexKey district_key =
        // GetDistrictPrimaryKey(stock_level_param->d_id_,
        // stock_level_param->w_id_); GAddr district_addr = 0;
        // DB_QUERY(SearchRecord(&context_, DISTRICT_TABLE_ID, district_key,
        // district_addr, READ_ONLY)); assert(district_addr != 0); RecordGAM
        // district_record; gallocators[thread_id_]->Read(district_addr,
        // &district_record, sizeof(RecordGAM)); int d_next_o_id = 0;
        // district_record.GetColumn(10, district_schema_ptr_, &d_next_o_id,
        // gallocators[thread_id_]); assert(d_next_o_id != 0);

        // size_t count = 0;
        // for (int o_id = d_next_o_id - 5; o_id < d_next_o_id; ++o_id){
        // 	// "getStockCount": "SELECT COUNT(DISTINCT(OL_I_ID)) FROM
        // ORDER_LINE, STOCK WHERE OL_W_ID = ? AND OL_D_ID = ? AND OL_O_ID < ?
        // AND OL_O_ID >= ? AND S_W_ID = ? AND S_I_ID = OL_I_ID AND S_QUANTITY <
        // ?" 	IndexKey order_line_key = GetOrderLineSecondaryKey(o_id,
        // stock_level_param->d_id_, stock_level_param->w_id_); 	GAddr
        // order_line_items_addr = 0; 	DB_QUERY(SearchRecords(&context_,
        // ORDER_LINE_TABLE_ID, 1, order_line_key, order_line_items_addr,
        // READ_ONLY)); 	ItemsIter iter(order_line_items_addr); 	GAddr record_addr
        // = 0; 	while ((record_addr = iter.GetNext(gallocators[thread_id_])) !=
        // 0){
        // 		++count;
        // 	}
        // }
