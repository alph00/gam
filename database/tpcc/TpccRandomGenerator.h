...
class TpccRandomGenerator {
   public:
    ...
    static void GenerateAddress(char *street_1, char *street_2, char *city,
                                char *state, char *zip) {
        std::string street1_str =
            TpccRandomGenerator::GenerateAString(MIN_STREET, MAX_STREET);
        memcpy(street_1, street1_str.c_str(), street1_str.size());
        std::string street2_str =
            TpccRandomGenerator::GenerateAString(MIN_STREET, MAX_STREET);
        memcpy(street_2, street2_str.c_str(), street2_str.size());
        std::string city_str =
            TpccRandomGenerator::GenerateAString(MIN_CITY, MAX_CITY);
        memcpy(city, city_str.c_str(), city_str.size());
        std::string state_str =
            TpccRandomGenerator::GenerateAString(STATE, STATE);
        memcpy(state, state_str.c_str(), state_str.size());
        int length = ZIP_LENGTH - ZIP_SUFFIX.size();
        std::string zip_str =
            TpccRandomGenerator::GenerateNString(length, length) + ZIP_SUFFIX;
    ...
    static int GenerateDistrictId(const int &num_districts_per_warehouse) {
        return TpccRandomGenerator::GenerateInteger(
            1, num_districts_per_warehouse);
    }
    ...
    static int GenerateItemId(const int &num_items) {
        return TpccRandomGenerator::GenerateNonuniformInteger(8191, 1,
                                                              num_items);
    }
