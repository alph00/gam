...
class TpccSource : public BenchmarkSource {
   ...
   private:
    ...
    virtual void StartGeneration() {
        ...
        std::cout << "read/write access count\ntotal_read_access_count\t"
    ...
    DeliveryParam *GenerateDeliveryParam(const int &w_id = -1) const {
        ...
        param->o_carrier_id_ = TpccRandomGenerator::GenerateInteger(
    ...
    NewOrderParam *GenerateNewOrderParam(const int &w_id = -1) {
        ...
        param->ol_cnt_ = static_cast<size_t>(
            TpccRandomGenerator::GenerateInteger(MIN_OL_CNT, MAX_OL_CNT));
        ...
        for (size_t i = 0; i < param->ol_cnt_; ++i) {
            ...
            param->i_qtys_[i] =
                TpccRandomGenerator::GenerateInteger(1, MAX_OL_QUANTITY);
        }
    ...
    void ModifyNewOrder(NewOrderParam *param) {
        ...
        for (size_t i = 0; i < param->ol_cnt_; ++i) {
            bool use_last_txn = (TpccRandomGenerator::GenerateInteger(1, 100) <=
                                 (int)gTimeLocality);
            if (use_last_txn &&
                new_order_access_items_.size() > use_last_txn_count) {
                // if decide to use items from last txn and last txn still has
                // unvisisted items, then randomly pick up one from last txn
                // access set
                while (true) {
                    int idx = TpccRandomGenerator::GenerateInteger(
                                  1, new_order_access_items_.size()) -
                              1;
                ...
                if (gForceRandomAccess) {
                    // if decide not to use items from last txn, but random
                    // access is required, still need to regenerate the item
                    while (true) {
                        int item_id =
                            TpccRandomGenerator::GenerateItemIdUniform(
                                scale_params_->num_items_);
                        // guarantee the uniqueness of the item.
                        if (exist_items.find(item_id) == exist_items.end()) {
                            param->i_ids_[i] = item_id;
            ...
            exist_items.insert(
                std::make_pair(param->i_ids_[i], param->i_w_ids_[i]));
        }
    ...
    PaymentParam *GeneratePaymentParam(const int &w_id = -1) {
        ...
        param->c_id_ = -1;
        // param->c_last_ = "";
        param->h_amount_ = TpccRandomGenerator::GenerateFixedPoint(
            2, MIN_PAYMENT, MAX_PAYMENT);
        param->h_date_ = TpccRandomGenerator::GenerateCurrentTime();

        bool remote =
            (TpccRandomGenerator::GenerateInteger(1, 100) <= (int)dist_ratio_);
        // 85%(by standard): paying through own warehouse (or there is only 1
        // warehouse)
        ...
        // currently, index cannot support lookup by name, forcely use lookup by
        // customer id
        y = 100;
    ...
    void ModifyPayment(PaymentParam *param) {
        ...
        // TODO(weihaosun): PaymentParam has a std::string-type field, memcpy
        // may cause memory fault
