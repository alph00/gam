...
class GlobalTimestamp {
   public:
    GlobalTimestamp(GAddr monotome_ts_addr)
        : monotome_ts_addr_(monotome_ts_addr) {}
    ...
    uint64_t GetBatchMonotoneTimestamp(GAlloc *gallocator) {
        ...
        gallocator->UnLock(monotome_ts_addr_, sizeof(uint64_t));
        return cur_ts;
        // return monotone_timestamp_.fetch_add(kBatchTsNum,
    ...
    // the purpose is (1) to collect garbage for version maintenance; (2)
    // generate a timestamp to retrieve consistent snapshot.

    // for OCC or 2PL, we can use maximum timestamp to retrieve consistent
    // snapshot. this is because the timestamp for OCC and 2PL is generated at
    // the commit time, and new committed transactions must have larger
    // timestamp.
    ...
    // this is because the timestamp for TO is generated at the beginning of a
    // transaction, and "staled" transactions can still commit.
    ...
    static void SetThreadTimestamp(const size_t &thread_id,
