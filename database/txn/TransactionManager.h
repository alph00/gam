...
class TransactionManager {
   public:
    TransactionManager(StorageManager* storage_manager,
                       TimestampManager* ts_manager, size_t thread_count,
                       size_t thread_id)
    ...
    bool InsertRecord(TxnContext* context, size_t table_id,
                      const IndexKey* keys, size_t key_num, Record* record,
    ...
    bool SearchRecord(TxnContext* context, size_t table_id,
        ...
        if (data_addr != Gnullptr) {
            ...
            epicLog(LOG_WARNING,
                    "table_id=%d cannot find the record with  key=%lx",
   ...
   private:
    ...
    bool TryRLockRecord(const GAddr& data_addr, size_t data_size) {
        epicLog(LOG_DEBUG,
                "TryRLockRecord, this=%p, data_addr=%lx, data_size=%d", this,
                data_addr, data_size);
        bool success = true;
        size_t try_count = 0;
    ...
    uint64_t GenerateScalableTimestamp(const uint64_t& curr_epoch,
                                       const uint64_t& max_rw_ts) {
        ...
        if (curr_epoch == max_global_ts) {
            if (this->local_ts_ <= max_local_ts) {
                this->local_ts_ =
                    (max_local_ts / thread_count_ + 1) * thread_count_ +
                    thread_id_;
                assert(this->local_ts_ > max_local_ts);
            }
            assert(this->local_ts_ > max_local_ts);
        }
        assert(this->local_epoch_ == max_global_ts &&
                   this->local_ts_ >= max_local_ts ||
               this->local_epoch_ > max_global_ts);
    ...
    uint64_t GenerateMonotoneTimestamp(const uint64_t& curr_epoch,
                                       const uint64_t& monotone_ts) {
