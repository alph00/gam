...
bool TransactionManager::InsertRecord(TxnContext *context, size_t table_id,
    ...
    original record. END_PHASE_MEASURE(thread_id_, CC_INSERT); return true;
    }*/
}
...
bool TransactionManager::CommitTransaction(TxnContext *context, TxnParam *param,
    ...
    for (size_t i = 0; i < access_list_.access_count_; ++i) {
        ...
        if (access_ptr->access_type_ == READ_ONLY) {
            BEGIN_GAM_OPERATION_MEASURE(thread_id_, GAM_RLOCK);
            RLockRecord(access_ptr->access_addr_,
                        table_record->GetSerializeSize());
            END_GAM_OPERATION_MEASURE(thread_id_, GAM_RLOCK);
            BEGIN_GAM_OPERATION_MEASURE(thread_id_, GAM_READ);
            ...
            if (content_ref.GetTimestamp() != access_ptr->timestamp_) {
                UPDATE_CC_ABORT_COUNT(thread_id_, context->txn_type_,
                                      access_ptr->table_id_);
                is_success = false;
                break;
            }
            ...
            WLockRecord(access_ptr->access_addr_,
                        table_record->GetSerializeSize());
