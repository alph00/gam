...
class ParamBatch {
   public:
    ...
    size_t size() const { return param_count_; }

    TxnParam *get(const size_t idx) const { return params_[idx]; }
...
class ParamBatchWrapper {
   public:
    ...
    ParamPtrWrapper *get(const size_t idx) const { return &(params_[idx]); }
