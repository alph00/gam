...
class ClusterConfig {
   public:
    ...
    ServerInfo GetMyHostInfo() const { return my_info_; }

    ServerInfo GetMasterHostInfo() const { return server_info_.at(0); }

    size_t GetPartitionNum() const { return server_info_.size(); }

    size_t GetMyPartitionId() const {
        for (size_t i = 0; i < server_info_.size(); ++i) {
            ServerInfo host = server_info_.at(i);
            if (host.addr_ == my_info_.addr_ &&
                host.port_no_ == my_info_.port_no_) {
                return i;
            }
        }
   ...
   private:
    void ReadConfigFile() {
        ...
        for (auto& entry : server_info_) {
            std::cout << "server name=" << entry.addr_
                      << ",port_no=" << entry.port_no_ << std::endl;
        }
    }
