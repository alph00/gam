...
struct BinaryKeyHash
    : public std::unary_function<std::tuple<T1, T2>, std::size_t> {
...
struct TernaryKeyHash
    : public std::unary_function<std::tuple<T1, T2, T3>, std::size_t> {
...
template <typename T1, typename T2, typename T3>
struct TernaryKeyEqual
    : public std::binary_function<std::tuple<T1, T2, T3>,
...
static uint32_t FastHash(const char *str) { return *(uint32_t *)(str); }
...
static std::string GetLogFilenameTs(const std::string &folder,
    ...
    boost::posix_time::ptime c_time =
        boost::posix_time::second_clock::local_time();
#ifdef _WIN32
    return folder + "\\" + prefix + "_" +
           boost::posix_time::to_iso_string(c_time) + ".log";
#elif __linux__
    return folder + "/" + prefix + "_" +
           boost::posix_time::to_iso_string(c_time) + ".log";
#endif
