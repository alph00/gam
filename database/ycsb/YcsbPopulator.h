...
private:
  ...
  void InsertMainTableRecord(int id, TableRecord *MainTable_record_buf,
    ...
    for (auto i = 1; i < record_buf->GetSchema()->GetColumnCount() - 1; ++i) {
      std::string data_s = YcsbRandomGenerator::GenerateAString(MIN_F1, MAX_F1);
      char *data_c = (char *)malloc(record_buf->GetSchema()->GetColumnSize(i));
      memcpy(data_c, data_s.c_str(), record_buf->GetSchema()->GetColumnSize(i));
