...
static uint64_t mstime(void) { return nstime() / 1000000; }
...
void populate(FILE* fp, kvClient* cli) {
    ...
    printf("%lu, %.2f\n", finished.load(), (finished - 1) * 1000 / duration);
}
...
int main(int argc, char* argv[]) {
    ...
    if (cache_th <= 100) {
        ...
        for (int i = 0; i < val_size; i++) value[i] = 'x';
        value[val_size - 1] = 0;
        ...
        for (i--; i >= 0; i--) pthread_join(threads[i], NULL);
