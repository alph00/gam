...
int kvClient::put(char* key, char* value) {
    ...
    while (1) {
        ...
        for (pos = 0; pos < BKT_SLOT; ++pos) {
            ...
            if (matchTag(p, TAG(kvp->hkey)) || !containTag(p)) break;
        }
        ...
        /// 当前bucket不存在此entry, 并且已经遍历完全, 并且没有后续bucket,
        /// 所以需要新分配一个物理bucket,
        /// 并且通过bucket内存的最后8个字节来存放新bucket的地址
        if (tmp == 0) {
            // allocator_->WLock(bktAddr, BKT_SIZE);
    ...
    if (size != kvp->size()) {
        // 如果更新了kv的实际物理内存地址,
        // 需要更新对应bucket中的对应entry的内容, 并更新bucket内存
        size = kvp->size();
        tag = TAG(kvp->hkey);
        updateEntry(p, tag, size, addr);
        allocator_->Write(bktAddr, bkt_, BKT_SIZE);
        // fprintf(stdout, "bktAddr = %lx, key = %lx, bucket = %lx, tag = %x,
        // size = %d, addr = %lx, p = %lx, bkt = %lx\n", bktAddr, kvp->hkey,
        // BUCKET(kvp->hkey), tag, size, addr, p, bkt_);
    }
