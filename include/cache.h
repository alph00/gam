...
// unordered_map<GAddr, CacheLine> caches;
    ...
    inline bool IsCachable(CacheLine* cline, WorkRequest* wr) {
        ...
        if (wr->flag & NOT_CACHE) {
            ...
            if (curTotalWR <= lim && totalWR <= lim) {
                ...
                idc = double(curClineWR) / curTotalWR + 1 -
                      double(curClineInvalid) / curTotalInvalid;
                if (idc >= thld) {
                    return true;
                } else {
                    return false;
   ...
   public:
    ...
    inline CacheState GetState(CacheLine* cline) { return cline->state; }
...
#ifdef SELECTIVE_CACHING
        if (cline->state ==
            CACHE_DIRTY) {  // dirty to shared (someone wants to read the data)
            cline->ntoshared++;
            ntoshared++;
        }
#endif
    ...
    inline bool IsDirty(GAddr addr) { return GetState(addr) == CACHE_DIRTY; }
    ...
    inline bool IsWLocked(CacheLine* cline, GAddr addr) {
        return cline->locks.count(addr) &&
               cline->locks.at(addr) == EXCLUSIVE_LOCK_TAG;
    }
