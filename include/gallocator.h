...
class GAlloc {
   ...
   public:
     ...
     * 				trying to put the newly-allocated addr to be in the same node of the
     * base addr
     */
    ...
    /*
     * Generally, write is non-blocking as we're using release memory
     * consistency model
     */
    ...
    inline int GetID() { return wh->GetWorkerId(); }
    int GetWorkersSize() { return wh->GetWorkersSize(); }

    /*
     * check whether addr is a local addr or not
     */
    inline bool IsLocal(GAddr addr) { return WID(addr) == GetID(); }
