...
class GArray {
    ...
    GAlloc* galloc_;
    GAddr header_ =
        Gnullptr;  // the global address storing this class information
    ...
    /*
     * locate the pointer address in layer "layer" that points to the "idx"th
     * block in layer "layer+1"
     */
    GAddr LocateInternal(GAddr root_addr, int total_layer, int layer,
                         size_t idx, GAlloc* galloc) {
    ...
    void SyncFrom(GAlloc* galloc) {
        ...
        if (header_) {  // update the header in case others resize the array
                        // (pessimistic)
    ...
    int Layers(size_t size) {
        return ceil(log((double)size / items_per_bucket) /
                    log((double)gaddrs_per_bucket)) +
               1;
    }
    ...
    inline void InsertBlock(GAddr root_addr, int total_layer, int l, size_t idx,
        ...
        GAddr addr = LocateInternal(
            root_addr, total_layer, l, idx,
            galloc);  // parent layer storing the pointer to next layer
   ...
   public:
    GArray(GAlloc* galloc, Flag flag = 0) : galloc_(galloc), flag_(flag){};
    GArray(const GArray& clone, GAlloc* galloc) : galloc_(galloc) {
        Clone(clone.Header(), galloc);
    }
    GArray(size_t size, GAlloc* galloc, Flag flag = 0)
        : size_(size), galloc_(galloc), flag_(flag) {
        if (size <= items_per_bucket) {
            ...
            if (layer_ >= 2) {
                epicLog(LOG_WARNING,
                        "array layer larger than 2 (array layer = %d)", layer_);
            }
    ...
    /*
     * Must be called after Globalize cos we have to lock in case there are
     * multiple resize operations
     */
    void Resize(size_t n, GAlloc* galloc = nullptr) {
        if (!galloc) galloc = galloc_;
        epicAssert(galloc);
        LockHeader(galloc);
        SyncFrom(galloc);
        if (n >
            size_) {  // enlarge: 1. allocate memory; 2. update header; 3. sync
            if (n > capacity_) {  // if n <= capacity_, nothing to do except
                                  // update the size_ member
                epicLog(LOG_WARNING, "have to resize from %lu to %lu", size_,
                        n);
                int layer = Layers(n);
                ...
                if (layer > layer_) {
                    ...
                    layer_ = layer;
                    root_addr_ = galloc->AlignedMalloc(BUCKET_SIZE, flag_);
                    epicLog(LOG_WARNING, "increased layer from %d to %d",
                            old_layer, layer);
                    ...
                    for (i = 1; i < layer - old_layer; i++) {
                        for (long j = 0; j < new_layers[i]; j++) {
                            GAddr newbk =
                                galloc->AlignedMalloc(BUCKET_SIZE, flag_);
                            InsertBlock(root_addr_, layer_, i, j, newbk,
                                        galloc);
                        }
                    }
            ...
            size_ = n;
            SyncTo(galloc);
        } else if (n < size_) {  // shrink: 1. update header; 2. sync; 3. free
                                 // memory
            ...
            for (int i = old_layer - 1; i > 0; i--) {
                for (int j = old_layers[i] - 1; j >= new_layers[i]; j--) {
                    GAddr parent =
                        LocateInternal(old_root, old_layer, i, j, galloc);
                    GAddr blk;
            ...
            epicLog(LOG_WARNING,
                    "There may be a race here, since the targeted resize size "
                    "(%lu) equals the original size (%lu)",
                    n, size_);
        }
    ...
    inline bool IsGlobal() { return header_ ? true : false; }
    ...
    inline int Layers() const { return layer_; }

    inline size_t Size() const { return size_; }

    inline size_t Capacity() const { return capacity_; }

    inline GAddr Header() const { return header_; }

    bool operator==(GArray<T, BUCKET_SIZE>& ga) { return Equal(ga); }
