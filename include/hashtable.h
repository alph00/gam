...
class HashTable : public cuckoohash_map<Key, T> {
   ...
   public:
    ...
    inline size_t count(Key key) { return this->contains(key); }
...
inline void HashTable<Key, T>::lock(Key key) {
    ...
    lock_[cuckoohash_map<Key, T, MurmurHasher<Key>>::hash_function()(key) %
        ...
        .lock();
#else
    lock_[cuckoohash_map<Key, T>::hash_function()(key) % lock_array_size]
        .lock();
#endif
...
inline bool HashTable<Key, T>::try_lock(Key key) {
    ...
    return lock_[cuckoohash_map<Key, T, CityHasher<Key>>::hash_function()(key) %
        ...
        .try_lock();
#elif defined(USE_MURMURHASH)
    return lock_[cuckoohash_map<Key, T, MurmurHasher<Key>>::hash_function()(
                     key) %
                 lock_array_size]
        .try_lock();
#else
    return lock_[cuckoohash_map<Key, T>::hash_function()(key) % lock_array_size]
        .try_lock();
#endif
    // epicLog(LOG_DEBUG, "locked %s", name.c_str());
}
...
class HashTable : public std::unordered_map<Key, T> {
   public:
    HashTable(string name = "DEFAULT_HASHTABLE_NAME") { this->name = name; }
    void lock(Key key);
    void unlock(Key key);
