...
// #else
#define epicLog(level, fmt, ...)                                       \
    _epicLog((char *)__FILE__, (char *)__func__, __LINE__, level, fmt, \
             ##__VA_ARGS__)
// #endif
...
#else
#define epicAssert(_e)                                                      \
    ((_e) ? (void)0                                                         \
          : (epicLog(LOG_WARNING, #_e " Assert Failed"), PrintStackTrace(), \
             assert(false)))
#endif
