...
#define MASTER_RDMA_CONN_STRLEN          \
    22 /** 4 bytes for lid + 8 bytes qpn \
        * + 8 bytes for psn              \
        * seperated by 2 colons */

#define WORKER_RDMA_CONN_STRLEN (MASTER_RDMA_CONN_STRLEN + 8 + 16 + 2)

#define MAX_CONN_STRLEN \
    (WORKER_RDMA_CONN_STRLEN + 4 + 1)  // 4: four-digital wid, 1: ':', 1: \0
...
class RdmaResource {
     ...
     */
    std::vector<bool> slots;  // the states of all the allocated slots (true:
                              // occupied, false: free)
   ...
   public:
    /*
     * isForMaster: true -- used to communicate with/by Master (no need to reg
     * whole mem space) false -- used to communicate among workers (reg whole
     * mem space + comm buf)
     */
...
class RdmaContext {
   private:
    ...
    int slot_head;
    int slot_tail;
    bool full;  // to differentiate between all free and all occupied slot_head
                // == slot_tail
    ...
    atomic<int> pending_msg;  // including both RDMA send and write/read that
                              // don't use the send buf
    ...
    inline ssize_t Rdma(RdmaRequest& r) {
        ...
        epicLog(LOG_DEBUG,
                "op = %d, src = %lx, len = %d, id = %d, signaled = %d, dest = "
                "%lx, imm = %u, oldval = %lu, newval = %lu\n"
                "src = %s",
                r.op, r.src, r.len, r.id, r.signaled, r.dest, r.imm, r.oldval,
                r.newval, r.src);
        return Rdma(r.op, r.src, r.len, r.id, r.signaled, r.dest, r.imm,
                    r.oldval, r.newval);
    }
   ...
   public:
    ...
    inline bool IsMaster() { return isForMaster; }
    const char* GetRdmaConnString();
    int SetRemoteConnParam(const char* remConn);

    inline uint32_t GetQP() { return qp->qp_num; }
    ...
    ssize_t Send(const void* ptr, size_t len, unsigned int id = 0,
                 bool signaled = false);
    inline int PostRecv(int n) { return resource->PostRecv(n); }
    int Recv();
