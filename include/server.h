...
class Server {
   private:
    ...
    HashTable<uint32_t, Client*> qpCliMap{
        "qpCliMap"};  // thread-safe as it is dynamic
    HashTable<int, Client*> widCliMap{
        "widCliMap"};  // store all the wid -> Client map
    UnorderedMap<int, Client*> widCliMapWorker{
        "widCliMapWorker"};  // only store the wid -> Client map excluding
                             // ClientServer
   ...
   public:
    ...
    inline int GetClusterSize() { return widCliMap.size(); }
    ...
    const string& GetIP() const { return conf->worker_ip; }

    int GetPort() const { return conf->worker_port; }
