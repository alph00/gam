...
#define dbprintf(fmt, ...)                                                 \
    _epicLog((char *)__FILE__, (char *)__func__, __LINE__, LOG_DEBUG, fmt, \
             ##__VA_ARGS__)
...
class SlabAllocator {
     ...
     * object 0 means error: can't store such a large object
     */

    unsigned int slabs_clsid(const size_t size);

   public:
    /** Init the subsystem. 1st argument is the limit on no. of bytes to
     allocate, 0 if no limit. 2nd argument is the growth factor; each slab will
     use a chunk size equal to the previous slab's chunk size times this factor.
    ...
    void *sb_aligned_calloc(size_t count, size_t size,
