...
#define GADD(addr, off) \
    ((addr) + (off))  // for now, we don't have any check for address overflow
...
struct Conf {
    bool is_master = true;  // mark whether current process is the master
                            // (obtained from conf and the current ip)
