...
struct RDMASendData {
    ...
    unsigned int id;
    bool signaled;
    RDMASendData(Client* cli, void* b, size_t l, unsigned int id = 0,
                 bool signaled = false)
...
class Worker : public Server {
     ...
     * 1) use pipe for each thread directly to transfer the pointer (then wqueue
     * is not needed)
     * -- too many wakeups?
     * 2) use a single pipe for all the threads (thread->worker), and process
     * all the requests once it is waked up
     * -- too much contention in the single pipe?
     * NOTE: both strategies require one pipe for each thread in
     * (worker->thread) direction in order to wake up individual thread
     */
    boost::lockfree::queue<WorkRequest*>*
        wqueue;  // work queue used to communicate with local threads
#ifdef ASYNC_RDMA_SEND
    ...
    atomic<unsigned int>
        wr_psn;  // we assume the pending works will not exceed INT_MAX
     ...
     */
    // unordered_map<GAddr, queue<pair<Client*, WorkRequest*>>>
    // to_serve_requests;
#ifdef USE_SIMPLE_MAP
    Map<GAddr, queue<pair<Client*, WorkRequest*>>*> to_serve_requests{
        "to_serve_requests"};
#else
    HashTable<GAddr, queue<pair<Client*, WorkRequest*>>*> to_serve_requests{
        "to_serve_requests"};
#endif
    ...
    HashTable<GAddr, queue<WorkRequest*>*> to_serve_local_requests{
        "to_serve_local_requests"};
#endif
     ...
     */
    // unordered_map<int, Fence> fences; //worker-side receive pipe -> fence
    // structure for that thread
    ...
    atomic<Size> ghost_size;  // the locally allocated size that is not synced
                              // with Master
   ...
   public:
     ...
     * 2) get a cached copy of the whole picture about the global memory
     * allocator
     */
    ...
    inline bool IsMFenced(Fence* fence, WorkRequest* wr) {
        return (fence->mfenced) && !(wr->flag & REPEATED) &&
               !(wr->flag & TO_SERVE) && !(wr->flag & FENCE);
    }
...
#ifdef USE_APPR_LRU
    inline long GetClock() { retrun global_clock_; }

    inline void SetClock(long time) { global_clock_ = time; }
#endif
#endif
...
class WorkerFactory {
   ...
   public:
    ...
    static Worker* CreateServer(const Conf& conf) {
        if (server) throw SERVER_ALREADY_EXIST_EXCEPTION;
