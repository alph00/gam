    ...
    for (GAddr i = start_blk; i < end;) {
        ...
        if ((cline = GetCLine(i))) {
            ...
            if (state == CACHE_TO_INVALID && READ == wr->op) {
                epicLog(LOG_INFO,
                        "cache is going to be invalid, but still usable for "
                        "read op = %d",
                        wr->op);
            ...
            // for WRITE, cannot allow since it may dirty the cacheline before
            // it finished transmission
            if (state == CACHE_TO_DIRTY && READ == wr->op &&
                IsBlockLocked(cline)) {
                epicAssert(!IsBlockWLocked(cline));
                epicLog(LOG_INFO,
                        "cache is going from shared to dirty, but still usable "
                        "for read op = %d",
                        wr->op);
                GAddr gs = i > start ? i : start;
                epicAssert(GMINUS(nextb, gs) > 0);
            ...
            if (unlikely(InTransitionState(state))) {
                epicLog(LOG_INFO,
                        "in transition state while cache read/write(%d)",
                        wr->op);
            ...
            // we can copy the data immediately since it will not be
            // over-written by remote node and also allow following read ops get
            // the latest data
            if (READ == wr->op) {
#ifdef SELECTIVE_CACHING
                cline->nread++;
                ...
                if (state != CACHE_DIRTY) {
                    ...
                    //        we comment below deadlock handle since we add it
                    //        the worker deadlock case 3
                    //					/*
                    //					 * below is used to avoid
                    //deadlock
                    //					 * when we are in transition state (want to get
                    //ownership) and read locked,
                    //					 * home node wants to invalidate it (home
                    //becomes in transition state).
                    //					 * both will block and the deadlock solution in
                    //worker.cc:1775 is not enough,
                    //					 * because read lock will block it
                    //forever
                    //					 * if the thread holding the lock wants to read
                    //the data (will be blocked
                    //					 * since it is in transition
                    //state)
                    //					 */
                    //					if(IsBlockLocked(i)) {
                    //						epicAssert(!IsBlockWLocked(i));
                    //						epicLog(LOG_INFO, "read locked while cache
                    //write(%d)", wr->op);
                    //						//we increase the counter in
                    //case
                    //						//we false call
                    //Notify() 						wr->counter++; 						unlock(i); 						wr->unlock();
                    //						//worker->to_serve_local_requests[i].push(wr);
                    //						worker->AddToServeLocalRequest(i,
                    //wr); 						return 1;
                    //					}
                    ...
                    // put submit request at last in case reply comes before we
                    // process afterwards works
                    worker->SubmitRequest(cli, lwr,
                ...
                epicLog(LOG_WARNING, "unknown op in cache operations %d",
                        wr->op);
                epicAssert(false);
            }
        } else {
...
int Cache::Lock(WorkRequest* wr) {
    ...
    if ((cline = GetCLine(i))) {
        ...
        if (InTransitionState(state)) {
            epicLog(LOG_INFO, "in transition state while cache read/write(%d)",
                    wr->op);
            wr->is_cache_hit_ = false;
            worker->AddToServeLocalRequest(i, wr);
            unlock(i);
        ...
        if (RLOCK == wr->op) {
            ...
            if (state != CACHE_DIRTY) {
                ...
                //        we comment below deadlock handle since we add it the
                //        worker deadlock case 3
                //				/*
                //				 * below is used to avoid
                //deadlock
                //				 * when we are in transition state
                //(want to get ownership) and read locked,
                //				 * home node wants to invalidate it
                //(home becomes in transition state).
                //				 * both will block and the deadlock
                //solution in worker.cc:deadlock case 1 is not enough,
                //				 * because read lock will block it
                //forever
                //				 * if the thread holding the lock
                //wants to read the data (will be blocked
                //				 * since it is in transition
                //state)
                //				 */
                //				if(IsBlockLocked(i)) {
                //					epicAssert(!IsBlockWLocked(i));
                //					epicLog(LOG_INFO, "read locked
                //while cache write(%d)", wr->op); 					if(wr->flag & TRY_LOCK) {
                //						return -1;
                //					} else {
                //						worker->to_serve_local_requests[i].push(wr);
                //						return 1;
        ...
        wr->is_cache_hit_ = false;
        WorkRequest* lwr = new WorkRequest(*wr);
        // we hide the fact that it is whether a lock op or read/write from the
        // remote side as lock is completely maintained locally
...
void Cache::LinkLRU(CacheLine* cline) {
    ...
    for (j = 0; j < sample_num; j++) {
        ...
        if (lru_locks_[i].try_lock()) {
            ...
            cline->prev = nullptr;
            cline->next = heads[i];
            if (cline->next) cline->next->prev = cline;
            heads[i] = cline;
            if (!tails[i]) tails[i] = cline;
            lru_locks_[i].unlock();
            break;
        }
    }
    if (j == sample_num) {
        ...
        if (j == LRU_NUM) {
            epicLog(LOG_WARNING,
                    "cannot link to any lru list (total lru list %d)", LRU_NUM);
            i = GetRandom(0, LRU_NUM);
            lru_locks_[i].lock();
            epicAssert(cline != heads[i]);
            epicAssert((heads[i] && tails[i]) || (!heads[i] && !tails[i]));
...
void Cache::UnLinkLRU(CacheLine* cline, int i) {
    ...
    if (cline->next) cline->next->prev = cline->prev;
    if (cline->prev) cline->prev->next = cline->next;
...
void Cache::Evict() {
    epicLog(LOG_INFO,
            "used_bytes = %ld, max_cache_mem = %ld,  BLOCK_SIZE = %ld, th = "
            "%lf, to_evicted = %ld",
    ...
    if (used > 0 && used > max_cache_mem) {
        ...
        epicLog(LOG_INFO,
                "tryng to evict %d, used = %ld, max_cache_mem = %ld, used > "
                "max_cache_mem = %d",
                n, used, max_cache_mem, used > max_cache_mem);
        int ret = Evict(n);
        if (ret < n) {
            epicLog(LOG_INFO, "only able to evict %d, but expect to evict %d",
                    ret, n);
        }
    }
}
 ...
 * 		   false if we don't have enough free space for n more cache
 * lines
 */
int Cache::Evict(int n) {
    long long used = used_bytes - to_evicted * BLOCK_SIZE;
    if (used < 0 || used <= max_cache_mem) return 0;
    ...
    for (i = 0; i < n; i++) {
        ...
        if (lru_locks_[lru_no].try_lock()) {
            ...
            while (to_evict) {  // only unlocked cache line can be evicted
                addr = to_evict->addr;
                if (try_lock(addr)) {
                    if (unlikely(to_evict->locks.size() ||
                                 InTransitionState(to_evict))) {
                        epicLog(LOG_INFO, "cache line (%lx) is locked",
                                to_evict->addr);
                        unlock(addr);
                    } else {
                        break;
            ...
            if (to_evict) {
                UnLinkLRU(
                    to_evict,
                    to_evict->pos);  // since we already got the lock in the
                                     // parent function of Evict(CacheLine*)
            }
            lru_locks_[lru_no].unlock();
            if (!to_evict) {
...
void Cache::Evict(CacheLine* cline) {
    ...
    if (CACHE_SHARED == state) {
        ...
        ToToInvalid(cline);
        //  UnLinkLRU(cline, pos); //since we already got the lock in the parent
        //  function of Evict(CacheLine*)
...
CacheLine* Cache::SetCLine(GAddr addr, void* line) {
    ...
    if (caches.count(block)) {
        ...
        if (line) {
            ...
            caddr ptr =
                worker->sb.sb_aligned_calloc(1, BLOCK_SIZE + CACHE_LINE_PREFIX);
            used_bytes += (BLOCK_SIZE + CACHE_LINE_PREFIX);
