...
int Client::SetRemoteConnParam(const char* conn) {
    ...
    } else if (IsForMaster()) {  // in the worker thread, but connected to
                                 // Master
        sscanf(conn, "%x:", &wid);
    } else if (!resource->IsMaster()) {  // in the worker thread, and connected
                                         // to worker
        sscanf(conn, "%x:", &wid);
    } else {
...
const char* Client::GetConnString(int workerid) {
    ...
    if (resource->IsMaster()) {                      // in the Master thread
        ...
        sprintf(
            connstr, "%04x:%s", workerid,
            rdmaConn);  // wid is the current worker id (not the remote pair's)
    } else {
        epicLog(LOG_WARNING, "undefined cases");
    }
...
Client::~Client() { resource->DeleteRdmaContext(ctx); }
