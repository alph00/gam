...
bool Directory::InTransitionState(ptr_t ptr) {
    ...
    if (dir.count(block)) {
        ...
        epicLog(LOG_DEBUG, "found the directory entry for %lx, state = %d",
                block, entry->state);
...
DirEntry* Directory::ToShared(void* ptr, GAddr addr) {
    // TODO: add a check on the max shared entries to 128 (i.e.,
    // MAX_UNSIGNED_CHAR)
    epicAssert((ptr_t)ptr == TOBLOCK(ptr));
    ptr_t block = TOBLOCK(ptr);
...
void Directory::ToDirty(DirEntry* entry, GAddr addr) {
    epicAssert(entry);
    epicAssert(entry->state == DIR_UNSHARED || entry->state == DIR_SHARED ||
...
void Directory::Clear(DirEntry*& entry, GAddr addr) {
    if (!entry) {
        epicLog(LOG_WARNING,
                "there must be a race before, and it has already been "
                "invalidated (%lx)",
                addr);
        return;
    }
    if (entry->state == DIR_DIRTY) {
        ...
        if (addr == entry->shared.front()) {
            ...
            epicLog(LOG_WARNING,
                    "there must be a race before, and new owner change from %d "
                    "to %d",
