...
void GatherPagerank(void *ptr, uint64_t argc) {
    // for simplicity and performance, we use push-based weight propagation
    // instead of pull-based gathering here, reduce value in vertex.accumulator
    // atomically
...
void ApplyPagerank(void *ptr, uint64_t argc) {
    ...
    if (vertex_data->active) {
        ...
        double new_pagerank =
            vertex_data->accumulator * (1.0 - REST_PROB) + REST_PROB;
        vertex_data->delta = new_pagerank - vertex_data->value;
        vertex_data->active_minor_step =
