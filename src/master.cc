...
Master::Master(const Conf& conf) : st(nullptr), workers(), unsynced_workers() {
    ...
    char* bind_addr = conf.master_bindaddr.length() == 0
                          ? nullptr
                          : const_cast<char*>(conf.master_bindaddr.c_str());
    ...
    switch (wr->op) {
        case UPDATE_MEM_STATS: {
            ...
            if (unsynced_workers.size() == conf->unsynced_th) {
                ...
                char buf[conf->unsynced_th * MAX_MEM_STATS_SIZE +
                         1];  // op + list + \0
                ...
                while (!unsynced_workers.empty()) {
                    ...
                    n += sprintf(buf + n, "%u:%d:%ld:%ld", lc->GetQP(),
                                 lc->GetWorkerId(), lc->GetTotalMem(),
                                 lc->GetFreeMem());
        ...
        case FETCH_MEM_STATS: {
            ...
            //		if(widCliMapWorker.size() == 1) { //only have the info of the
            //worker, who sends the request 			break;
            //		}
            WorkRequest lwr{};
            lwr.op = FETCH_MEM_STATS_REPLY;
            char buf[(widCliMapWorker.size()) * MAX_MEM_STATS_SIZE +
                     1];  // op + list + \0
        ...
        case PUT: {
            ...
            if (to_serve_kv_request.count(wr->key)) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    epicLog(LOG_DEBUG,
                            "processed to-serve remote kv request for key %ld",
                            to_serve.second->key);
        ...
        case GET: {
            if (kvs.count(wr->key)) {
                ...
                if (send_buf == nullptr) {
                    ...
                    send_buf = (char*)zmalloc(MAX_REQUEST_SIZE);
                    epicLog(LOG_INFO,
                            "We don't have enough slot buf, we use local buf "
                            "instead");
                }
