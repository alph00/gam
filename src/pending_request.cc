...
void Worker::ProcessPendingRead(Client* cli, WorkRequest* wr) {
    ...
    epicAssert((IsLocal(wr->addr) && wr->op == FETCH_AND_SHARED) ||
    ...
    if (!(wr->flag & REPEATED)) {
        if ((wr->flag &
             CACHED)) {  // read is issued by the cache (remote memory)
            ...
            cache.ToShared(cline);
#endif
        } else if (IsLocal(wr->addr)) {  // read is issued by local worker
                                         // (local memory)
    ...
    if (wr->flag & LOCKED) {  // RLOCK
        epicAssert(!(wr->flag & NOT_CACHE) && wr->addr == blk &&
                   wr->size == BLOCK_SIZE);
...
void Worker::ProcessPendingWrite(Client* cli, WorkRequest* wr) {
...
#endif
    epicAssert((wr->op == WRITE || wr->op == WRITE_PERMISSION_ONLY) xor
               IsLocal(wr->addr));
    WorkRequest* parent;
    CacheLine* cline = nullptr;
    DirEntry* entry = nullptr;
    ...
    if (wr->status) {  // failed from one of the responders
        ...
        if ((wr->flag &
             CACHED)) {  // write is issued by the cache (remote memory)
            epicAssert(wr->op == WRITE || wr->op == WRITE_PERMISSION_ONLY);
            epicAssert(!IsLocal(wr->addr));
        } else if (IsLocal(wr->addr)) {  // write is issued by local worker
                                         // (local memory)
            ...
            epicAssert(directory.GetState(entry) !=
                       DIR_UNSHARED);  // not possible to erase the entry
        } else {
            epicLog(LOG_WARNING, "unexpected");
            epicAssert(false);
        }
        ...
        if (wr->counter == 0) {
            ...
            --wr->parent->counter;
            epicAssert(wr->parent->counter ==
                       0);     // lock is guaranteed to be only one block
            parent->unlock();  // unlock earlier
            // Notify() should be called in the very last after all usage of
            // parent, since the app thread may exit the function and release
            // the memory of parent
            Notify(wr->parent);
            wr->parent = nullptr;
    ...
    if ((wr->flag & CACHED)) {  // write is issued by the cache (remote memory)
        ...
        epicAssert(!IsLocal(wr->addr));
        epicAssert(!IsLocal(wr->addr));
    } else if (IsLocal(wr->addr)) {  // write is issued by local worker (local
                                     // memory)
        epicAssert(wr->ptr == ToLocal(wr->addr));
        epicAssert(directory.GetState(entry) == DIR_TO_UNSHARED);
        directory.Remove(entry, cli->GetWorkerId());
    ...
    if (wr->counter == 0 || (wr->flag & REQUEST_DONE)) {
        ...
        if (!(wr->flag & NOT_CACHE)) {
            ...
            if (!(wr->flag & LOCKED)) {
                ...
                GAddr gs = wr->addr > parent->addr ? wr->addr : parent->addr;
                void* ls =
                    (void*)((ptr_t)parent->ptr + GMINUS(gs, parent->addr));
                void* cs = (void*)((ptr_t)wr->ptr + GMINUS(gs, wr->addr));
                Size len = end > pend ? GMINUS(pend, gs) : GMINUS(end, gs);
                // we blindly copy the data again
                // as while we are waiting for the reply,
                // there may be a race causes the current op to be canceled or
                // renamed (WRITE_PERMISSION_ONLY to WRITE)
#ifdef GFUNC_SUPPORT
                if (wr->flag & GFUNC) {
                    epicAssert(wr->gfunc);
        ...
        if (wr->flag & LOCKED) {  // WLOCK
            epicLog(LOG_DEBUG, "parent->op = %d, parent->counter = %d",
                    parent->op, parent->counter.load());
            epicAssert(WLOCK == parent->op && 1 == parent->counter);
        }

        // TODO: we should process the to_serve requests and pending requests
        // first then process the fenced requests for now, it's ok since write
        // will prevent fenced requests to be processed and read is blocking
        bool notify = false;
...
void Worker::ProcessPendingWriteForward(Client* cli, WorkRequest* wr) {
    ...
    if (wr->flag & NOT_CACHE) {
        ...
        if (wr->flag & GFUNC) {
            ...
            epicAssert(TOBLOCK(parent->addr) ==
                       TOBLOCK(GADD(parent->addr, parent->size - 1)));
        ...
        lcli->WriteWithImm(nullptr, nullptr, 0,
                           wr->pid);  // ack the ownership change
...
void Worker::ProcessPendingInvalidateForward(Client* cli, WorkRequest* wr) {
    ...
    if (wr->status) {  // failed from one of the responders
        ...
        if (wr->counter == 0) {
            epicLog(LOG_INFO,
                    "INVALIDATE_FORWARD: failed case final-processing");
            void* laddr = ToLocal(wr->addr);
            epicAssert(directory.GetState(entry) == DIR_TO_DIRTY);
            // undo the directory/cache changes
            directory.UndoShared(entry);
            ...
            parent->op = WRITE_REPLY;
            // comment below as counter is not initialized and not used in the
            // write/invalidforward parent->counter--;
            ...
            delete wr;
            wr = nullptr;
            // parent has been already deleted when we receive failed response
            // the first time
        } else {
            wr->unlock();
            directory.unlock(ToLocal(wr->addr));
    ...
    if (wr->counter == 0) {
        ...
        if (wr->flag & NOT_CACHE) {
            ...
            if (WRITE == parent->op) {
                ...
                // deadlock: one node (Node A) wants to update its cache from
                // shared to dirty, but at the same time, the home nodes
                // invalidates all its shared copy (due to a local write, or
                //remote write after local/remote read) currently, dir_state ==
                // DIR_UNSHARED which means that the shared list doesn't contain
                // the requesting node A. solution: Node A acts as it is still a
                // shared copy so that the invalidation can completes, after
                // which, home node processes the pending list and change the
                // processing from WRITE_PERMISSION_ONLY to WRITE
            ...
            if (entry) {
                directory.ToDirty(
                    entry,
                    lcli->ToGlobal(parent->ptr));  // entry should be null
            } else {
                directory.ToDirty(
                    laddr,
                    lcli->ToGlobal(parent->ptr));  // entry should be null
