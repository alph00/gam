...
int RdmaResource::RegLocalMemory(void* base, size_t sz) {
    ...
    if (this->base || this->bmr || isForMaster) {
        epicLog(LOG_WARNING,
                "An mr has already be registered or I am a master\n");
        return ret;
    }
...
int RdmaResource::RegCommSlot(int slot) {
    ...
    if (slots.size() - slot_inuse >= slot) {
        epicLog(LOG_DEBUG,
                "no need to register: inuse = %d, current slots = %d\n",
        ...
        for (; i < slot_inuse; i += RECV_SLOT_STEP) {
            ...
            void* buf = zmalloc(sz);
            struct ibv_mr* mr =
                ibv_reg_mr(this->pd, buf, sz,
                           IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE |
                               IBV_ACCESS_REMOTE_READ);
            if (!mr) {
                epicLog(LOG_FATAL,
                        "Unable to register mr for communication slots");
                return -1;
            }
...
RdmaResource* RdmaResourceFactory::GetRdmaResource(bool isMaster,
                                                   const char* devName) {
    if (!devName) {
...
RdmaContext::RdmaContext(RdmaResource* res, bool master)
    : resource(res),
      ...
      pending_msg(0),
      pending_send_msg(0) {
    // check either master == true, or both isMaster() in RdmaContext and
    // RdmaResouce are false
...
const char* RdmaContext::GetRdmaConnString() {
     ...
     * for communication among workers, we also allow direct access to the whole
     * memory space so that we expose the base addr and rkey
     */
    if (IsMaster()) {
...
char* RdmaContext::GetFreeSlot_() {
    int avail = RMINUS(
        slot_tail, slot_head,
        max_pending_msg);  // slot_head <= slot_tail ? slot_tail-slot_head :
                           // slot_tail+max_pending_msg-slot_head;
    if (!avail && !full) avail = max_pending_msg;
    epicLog(LOG_DEBUG, "avail = %d, pending_msg = %d", avail,
            pending_msg.load());
...
ssize_t RdmaContext::Rdma(ibv_wr_opcode op, const void* src, size_t len,
    ...
    epicLog(LOG_DEBUG,
            "op = %d, src = %lx, len = %d, id = %d, signaled = %d, dest = %lx, "
            "imm = %u, oldval = %lu, newval = %lu\nsrc = %s",
    ...
    if (pending_msg >= max_pending_msg) {
        ...
        pending_requests.push(
            RdmaRequest{op, src, len, id, signaled, dest, imm, oldval, newval});
        epicAssert(pending_requests.back().op == op &&
                   pending_requests.back().src == src &&
    ...
    if (op == IBV_WR_SEND) {
        if (unlikely(!IsRegistered(src) && len > MAX_RDMA_INLINE_SIZE)) {
            if (len > MAX_REQUEST_SIZE) {
                epicLog(LOG_WARNING,
                        "len = %d, MAX_REQUEST_SIZE = %d, src = %s\n", len,
                        MAX_REQUEST_SIZE, src);
                epicAssert(false);
            }
    ...
    pending_msg++;
    uint16_t curr_to_signaled_send_msg =
        pending_send_msg - to_signaled_send_msg;
    uint16_t curr_to_signaled_w_r_msg =
        pending_msg - pending_send_msg - to_signaled_w_r_msg;

    if (unlikely(curr_to_signaled_send_msg + curr_to_signaled_w_r_msg ==
                     max_unsignaled_msg ||
        ...
        epicAssert(curr_to_signaled_send_msg + curr_to_signaled_w_r_msg <=
                   max_unsignaled_msg);
...
ssize_t RdmaContext::WriteWithImm(raddr dest, raddr src, size_t len,
    ...
    lock();
    ssize_t ret =
        Rdma(IBV_WR_RDMA_WRITE_WITH_IMM, src, len, id, signaled, dest, imm);
    unlock();
    return ret;
}
...
void RdmaContext::ProcessPendingRequests(int n) {
    ...
    // we must iterate all the current pending requests in order to ensure the
    // original order: not necessary (updated)
    int i = 0, j = -1;
    ...
    for (i = 0; i < size; i++) {
        ...
        if (r.op == IBV_WR_SEND) {
            ...
            if (prev_wr && prev_wr->opcode == IBV_WR_SEND &&
                ...
                *((char*)prev_wr->sg_list->addr + buf_pos) = '\0';  // sep by \0
                memcpy((void*)((char*)prev_wr->sg_list->addr + buf_pos + 1),
                       r.src, r.len);
                zfree((void*)r.src);
                buf_pos += (1 + r.len);
        ...
        if (unlikely(curr_to_signaled_send_msg + curr_to_signaled_w_r_msg ==
            ...
            epicAssert(to_signaled_send_msg == pending_send_msg);
            to_signaled_w_r_msg += curr_to_signaled_w_r_msg;
            epicAssert(to_signaled_send_msg + to_signaled_w_r_msg ==
                       pending_msg);
            epicAssert(curr_to_signaled_send_msg + curr_to_signaled_w_r_msg <=
                       max_unsignaled_msg);
            wrs[j].wr_id =
    ...
    if (size) {
        if (i == size) {
            ...
            j++;  // so that wrs[j-1] is the last valid request even if we
                  // consume all the pending requests
        } else {
...
unsigned int RdmaContext::SendComp(ibv_wc& wc) {
    ...
    ProcessPendingRequests(curr_to_signaled_send_msg +
                           curr_to_signaled_w_r_msg);
...
unsigned int RdmaContext::WriteComp(ibv_wc& wc) { return SendComp(wc); }
