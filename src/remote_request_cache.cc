...
void Worker::ProcessRemoteRead(Client* client, WorkRequest* wr) {
    ...
    if (directory.InTransitionState(entry)) {
        // to_serve_requests[wr->addr].push(pair<Client*, WorkRequest*>(client,
        // wr));
    ...
    if (directory.GetState(entry) !=
        ...
        if (directory.IsBlockWLocked(entry)) {
            ...
            if (wr->flag & TRY_LOCK) {  // reply directly with lock failed
                ...
                // to_serve_requests[wr->addr].push(pair<Client*,
                // WorkRequest*>(client, wr));
                AddToServeRemoteRequest(wr->addr, client, wr);
                directory.unlock(laddr);
            }
            return;
    ...
    if (!cline) {
        ...
        if (cache.IsBlockWLocked(cline)) {
            if (wr->flag & TRY_LOCK) {  // reply directly with lock failed
                ...
                epicAssert(!deadlock);
                // we must unlock the cache/directory lock before calling the
                // AddToServe[Remote]Request as the lock acquire seq is fences
                // -> directory/cache ->
                // to_serve_local/remote_request/pending_works the
                // ProcessToServeRequest() breaks this rule we copy the queue
                // first and then release the to_serve.._request lock
                // immediately to_serve_requests[wr->addr].push(pair<Client*,
                // WorkRequest*>(client, wr));
                AddToServeRemoteRequest(wr->addr, client, wr);
                cache.unlock(blk);
            }
        ...
        if (op_orin == FETCH_AND_SHARED) {
...
#endif
            epicAssert(BLOCK_ALIGNED(wr->addr) || wr->size < BLOCK_SIZE);
            client->WriteWithImm(wr->ptr, cline->line, wr->size,
                                 wr->id);  // reply to the local home node
        } else {                           // READ_FORWARD
            Client* cli = FindClientWid(wr->pwid);
            ...
            epicAssert(BLOCK_ALIGNED(wr->addr) || wr->size < BLOCK_SIZE);
            cli->WriteWithImm(wr->ptr, cs, wr->size,
                              wr->pid);  // reply to the local node
            if (!(wr->flag & NOT_CACHE)) {
                epicAssert(BLOCK_ALIGNED(wr->addr) || wr->size < BLOCK_SIZE);
                client->WriteWithImm(client->ToLocal(blk), cline->line,
                                     BLOCK_SIZE,
                                     wr->id);  // writeback to home node
            }
#else
            epicAssert(BLOCK_ALIGNED(wr->addr) || wr->size < BLOCK_SIZE);
            cli->WriteWithImm(wr->ptr, cline->line, wr->size,
                              wr->pid);  // reply to the local node
        ...
        if (!(wr->flag & NOT_CACHE)) {
            ...
            if (!deadlock) cache.ToShared(cline);
#ifdef SELECTIVE_CACHING
        }
#endif
    }
...
void Worker::ProcessRemoteWrite(Client* client, WorkRequest* wr) {
    ...
    if (state != DIR_DIRTY) {
        ...
        if (directory.IsBlockLocked(entry)) {
            epicAssert(
                (directory.IsBlockWLocked(entry) && state == DIR_UNSHARED) ||
        ...
        if (state == DIR_SHARED) {
            // change the invalidate strategy (home node accepts invalidation
            // responses) in order to simply the try_lock failed case
            ...
            if (wr->flag & NOT_CACHE) {
                ...
                lwr->ptr =
                    (void*)((ptr_t)lwr->ptr -
                            GMINUS(wr->addr, lwr->addr));  // not necessary
            }
#endif
            ...
            for (auto it = shared.begin(); it != shared.end(); it++) {
                ...
                epicLog(LOG_DEBUG,
                        "invalidate forward (%d) cache from worker %d", lwr->op,
                        cli->GetWorkerId());
                if (first) {
            ...
            if (wr->flag & NOT_CACHE) {
                ...
                if (WRITE == op_orin) {
                    epicLog(LOG_DEBUG,
                            "write the data (size = %ld) to destination",
                            wr->size);
                    epicAssert(BLOCK_ALIGNED(wr->addr) ||
                               wr->size < BLOCK_SIZE);
                    ...
                    // deadlock: one node (Node A) wants to update its cache
                    // from shared to dirty, but at the same time, the home
                    // nodes invalidates all its shared copy (due to a local
                    // write) currently, dir_state == dir_unshared (after pend
                    // the request because it was dir_to_unshared) solution:
                    // Node A acts as it is still a shared copy so that the
                    // invalidation can completes, after which, home node
                    // processes the pending list and change the
                    // WRITE_PERMISSION_ONLY to WRITE
        ...
        if (!(wr->flag & NOT_CACHE)) {
            ...
            // we can safely change the directory as we've already transfered
            // the data to the local node
            //  logging
            logOwner(client->GetWorkerId(), wr->addr);
            if (entry) {
                directory.ToDirty(entry, client->ToGlobal(wr->ptr));
            } else {
                directory.ToDirty(laddr,
                                  client->ToGlobal(wr->ptr));  // entry is null
            }
        ...
        if (WRITE == op_orin || WLOCK == op_orin) {
            ...
            // deadlock: WRITE_PERMISSION_ONLY shouldn't co-exist with DIR_DIRTY
            // state there must be a race where one nodes (Node A) tries to
            // update its cache from shared to dirty, while another node (Node
            // B) writes the data before that node solution: Node A replies as
            // its cache line is shared, and home node changes it to
            // WRITE_FORWARD lwr->op = WRITE_PERMISSION_ONLY_FORWARD;
            lwr->op = WRITE_FORWARD;
        }
        lwr->parent = wr;
        lwr->pid = wr->id;
    ...
    if (!cline) {
        if (INVALIDATE == op_orin || INVALIDATE_FORWARD == op_orin) {
            ...
            if (wr->op == INVALIDATE) {  // INVALIDATE
                ...
                //			Client* cli = FindClientWid(wr->pwid);
                //			cli->WriteWithImm(nullptr, nullptr, 0,
                //wr->pid); //reply the new owner 			epicLog(LOG_DEBUG, "send to %d
                //with pid %d", wr->pwid, wr->pid);
                //      after change the invalidate_forward strategy
                client->WriteWithImm(nullptr, nullptr, 0, wr->id);
                epicLog(LOG_DEBUG, "send to %d with id %d",
                        client->GetWorkerId(), wr->id);
            }
        } else {
            epicLog(LOG_FATAL, "Unexpected: cannot find an updated copy");
        ...
        if (cache.InTransitionState(cline->state)) {
            /*
             * deadlock, since the responding node must just change its cache
             * state and send request to home node, who was not notified of the
             * change and sent an invalidate/forward request. How to solve?
            ...
            if ((INVALIDATE == wr->op || INVALIDATE_FORWARD == wr->op) &&
                ...
                if (cline->state == CACHE_TO_INVALID) {
                    ...
                    AddToServeRemoteRequest(wr->addr, client, wr);
                    epicLog(LOG_INFO, "cache in transition state %d",
                            cline->state);
        ...
        if (cache.IsBlockLocked(cline)) {
            if (wr->flag & TRY_LOCK) {  // reply directly with lock failed
                ...
                if (INVALIDATE == op_orin || FETCH_AND_INVALIDATE == op_orin) {
                    ...
                    //				Client* cli =
                    //FindClientWid(wr->pwid); 				wr->id = wr->pid;
                    //				SubmitRequest(cli, wr);
                    SubmitRequest(client, wr);
                } else {  // WRITE_FORWARD or WRITE_PERMISSION_ONLY_FORWARD
                    SubmitRequest(client, wr);
                ...
                // deadlock case 3
                // if it is rlocked, and in deadlock status (in transition state
                // from shared to dirty) we are still safe to act as it was in
                // shared state and ack the invalidation request because the
                // intransition state will block other r/w requests until we get
                // replies from the home node (then WRITE_PERMISSION_ONLY has
        ...
        if (wr->op == FETCH_AND_INVALIDATE) {  // FETCH_AND_INVALIDATE
            ...
            if (deadlock) {
                ...
                //			client->WriteWithImm(wr->ptr, line,
                //wr->size, wr->id); 			cache.ToInvalid(wr->addr); 			delete wr;
                ...
                epicAssert(BLOCK_ALIGNED(wr->addr) || wr->size < BLOCK_SIZE);
                client->WriteWithImm(wr->ptr, cline->line, wr->size, orig_id,
                                     wr->id, true);
            }
            ...
            client->WriteWithImm(nullptr, nullptr, 0, wr->id);
            // TOOD: add below to the callback function
            if (!deadlock) cache.ToInvalid(cline);
            delete wr;
            wr = nullptr;
        } else if (wr->op == INVALIDATE_FORWARD) {  // INVALIDATE_FORWARD
            epicAssert(!cache.IsDirty(cline));
            //		Client* cli = FindClientWid(wr->pwid);
            //		cli->WriteWithImm(nullptr, nullptr, 0, wr->pid); //reply the
            //new owner 		epicLog(LOG_DEBUG, "send to %d with pid %d", wr->pwid,
            //wr->pid);
            client->WriteWithImm(nullptr, nullptr, 0, wr->id);
            ...
            if (deadlock) {
                ...
                if (wr->flag & NOT_CACHE) {
                    // client->WriteWithImm(wr->ptr, cline->line, wr->size,
                    // wr->id);  //transfer ownership fix bug here (wr->ptr is
                    // not the same as ToLocal(wr->addr) and here we write the
                    // dirty data back to the home node rather than the local
                    // node requesting the data
                    epicAssert(BLOCK_ALIGNED(wr->addr));
                    epicAssert(BLOCK_ALIGNED(wr->addr) ||
                               wr->size < BLOCK_SIZE);
                    client->WriteWithImm(client->ToLocal(wr->addr), cline->line,
                                         wr->size,
                                         wr->id);  // transfer ownership
                } else {
#endif
                    epicAssert(BLOCK_ALIGNED(wr->addr) ||
                               wr->size < BLOCK_SIZE);
                    cli->WriteWithImm(wr->ptr, cline->line, wr->size,
                                      wr->pid);  // reply the new owner
                    epicLog(LOG_DEBUG, "send to %d with pid %d", wr->pwid,
                            wr->pid);
                    client->WriteWithImm(nullptr, nullptr, 0,
                                         wr->id);  // transfer ownership
#ifdef SELECTIVE_CACHING
                }
#endif
                ...
                //		  cli->WriteWithImm(wr->ptr, line, wr->size,
                //wr->pid); //reply the new owner 		  epicLog(LOG_DEBUG, "send to %d
                //with pid %d", wr->pwid, wr->pid);
                //			client->WriteWithImm(nullptr, nullptr, 0,
                //wr->id); //transfer ownership 			cache.ToInvalid(wr->addr);
                //			delete wr;
                unsigned int orig_id = wr->id;
                ...
                if (wr->flag & NOT_CACHE) {
                    ...
                    epicAssert(BLOCK_ALIGNED(wr->addr) ||
                               wr->size < BLOCK_SIZE);
                    client->WriteWithImm(client->ToLocal(wr->addr), cline->line,
                                         wr->size, orig_id, wr->id,
                                         true);  // transfer ownership
                } else {
#endif
                    epicAssert(BLOCK_ALIGNED(wr->addr) ||
                               wr->size < BLOCK_SIZE);
                    cli->WriteWithImm(wr->ptr, cline->line, wr->size, wr->pid,
                                      wr->id,
                                      true);  // reply the new owner
                    client->WriteWithImm(nullptr, nullptr, 0,
                                         orig_id);  // transfer ownership
#ifdef SELECTIVE_CACHING
                }
#endif
            }
...
void Worker::ProcessRemoteWriteReply(Client* client, WorkRequest* wr) {
    ...
    if (wr->status) {
        ...
        epicAssert(LOCK_FAILED ==
                   wr->status);  // for now, only this should happen
        epicAssert((pwr->flag & LOCKED) && (pwr->flag & TRY_LOCK));
        ...
        switch (pwr->op) {
            ...
            {
                if (WID(pwr->addr) ==
                    client->GetWorkerId()) {  // from home node, Case 4
                    epicAssert(
                        !wr->counter);  // must be 0 since the home node has
                                        // broadcast forward/invalidate req to
                                        // other remote nodes
                    epicAssert(WRITE == pwr->op ||
                               WRITE_PERMISSION_ONLY == pwr->op);
                    pwr->counter = 0;
                } else {
                    pwr->counter--;
                }
                ...
                if (pwr->counter == 0) {
                    ...
                    --pwr->parent->counter;
                    epicAssert(pwr->parent->counter ==
                               0);  // lock is guaranteed to be only one block
                    parent->unlock();  // unlock earlier
                    // Notify() should be called in the very last after all
                    // usage of parent, since the app thread may exit the
                    // function and release the memory of parent
                    Notify(pwr->parent);
                    pwr->parent = nullptr;
            ...
            {
                void* laddr =
                    ToLocal(pwr->addr);  // ToLocal(pwr->addr) != pwr->ptr as it
                                         // is a forward msg
                // directory.lock(laddr);
                ...
                lcli->WriteWithImm(nullptr, nullptr, 0,
                                   pwr->pid);  // ack the ownership change
                ...
                // following replies will be ignored since it cannot find
                // pending wr in the pending list ANSWER: it's ok here, since we
                // are sure that we only have one response for these two ops
                int ret = ErasePendingWork(wr->id);
                epicAssert(ret);
                delete pwr;
                pwr = nullptr;
            ...
            case INVALIDATE_FORWARD: {
                ...
                WorkRequest* parent = pwr->parent;
                epicAssert(parent);
                epicAssert((parent->flag & LOCKED) &&
                           (parent->flag & TRY_LOCK));
                pwr->status = wr->status;
