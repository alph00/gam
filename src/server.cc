...
void Server::ProcessRdmaRequest(ibv_wc& wc) {
    ...
    if (wc.status != IBV_WC_SUCCESS) {
        epicLog(LOG_WARNING, "Completion with error, op = %d (%d:%s)",
                wc.opcode, wc.status, ibv_wc_status_str(wc.status));
        epicAssert(false);
        return;
    }
    ...
    switch (wc.opcode) {
        ...
        case IBV_WC_RECV: {
            ...
            // we cannot use split by string since GET/PUT is using binary
            // protocol!!!
            ...
            while (consumed_len < wc.byte_len) {
                ...
                if (wr->Deser(data + consumed_len, len)) {
                    epicLog(LOG_WARNING,
                            "de-serialize the work request failed\n");
                } else {
                ...
                if (consumed_len < wc.byte_len) {
                    if (data[consumed_len] != '\0') {
                        epicLog(LOG_WARNING,
                                "received: pos = %d, len = %d, consumed_len = "
                                "%d, byte_len = %u, str = %s",
                                data[consumed_len], len, consumed_len,
                                wc.byte_len, data);
                        epicAssert(false);
                    }
                }
            ...
            if (wr->Deser(data, len)) {
                ...
                if (len != wc.byte_len) {
                    epicLog(LOG_WARNING,
                            "len = %d, wc.byte_len = %d, data = %s", len,
                            wc.byte_len, data);
                    epicAssert(false);
                }
                ProcessRequest(cli, wr);
...
void Server::ProcessRdmaRequest() {
     ...
     * we need ibv_req_notify_cq -> ibv_get_cq_event -> ibv_ack_cq_events seq ->
     * ibv_req_notify_cq!!
     */
...
void Server::UpdateWidMap(Client* cli) { widCliMap[cli->GetWorkerId()] = cli; }
...
void Server::RmClient(Client* c) {
    ...
    epicAssert(widCliMapWorker.count(c->GetWorkerId()) ==
               0);  // FIXME: assume no worker will be deleted for now
    // UNLOCK_MICRO(qpCliMap, 0);
}
