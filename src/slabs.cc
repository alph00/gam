...
void *SlabAllocator::mmap_malloc(size_t size) {
    ...
    if (size % BLOCK_SIZE) {
        ...
        epicLog(LOG_WARNING, "aligned the size from %lu to %lu", old_size,
                size);
    }
#ifdef USE_HUGEPAGE
...
#else
    ret = mmap(fixed_base, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANON,
               -1, 0);
#endif
    if (ret == MAP_FAILED) {
        perror("map failed");
    ...
    //		epicLog(LOG_FATAL, "allocate memory %ld failed (%d:%s)", size, errno,
    //strerror(errno));
    //	}
    return ret;
}
...
size_t SlabAllocator::get_avail() { return mem_free; }
...
void *SlabAllocator::slabs_init(const size_t limit, const double factor,
    ...
    while (++i < POWER_LARGEST && size <= item_size_max / factor) {
        ...
        if ((int)(pre_size / BLOCK_SIZE) < (int)(size / BLOCK_SIZE) &&
            ...
            // epicLog(LOG_DEBUG, "aligned slab to slabclass[i].size = %d",
            // slabclass[i].size);
            i++;
        }
...
void SlabAllocator::do_slabs_free(void *ptr, const size_t size,
    ...
    if (id < POWER_SMALLEST || id > power_largest) return;
...
void *SlabAllocator::sb_malloc(size_t size) {
    ...
    if (size > item_size_max) {
        ...
        epicLog(LOG_WARNING,
                "allocate memory %lu, larger than default max %d, at %lx", size,
                item_size_max, ret);
...
void *SlabAllocator::sb_aligned_malloc(size_t size, size_t block) {
     ...
     */
    if (size > item_size_max) {
        epicLog(LOG_WARNING, "allocate memory %lu, larger than default max %lu",
                size, item_size_max);
        lock();
        void *ret = memory_allocate(size);
        epicAssert((uint64_t)ret % BLOCK_SIZE == 0);
...
void *SlabAllocator::sb_realloc(void *ptr, size_t size) {
    ...
    if (ptr == NULL) return sb_malloc(size);
    ...
    if (id1 == id2) {
        ...
        // need to clear the clsid as the original memcached implementation
        // check this in free function
        it1->slabs_clsid = 0;
...
SlabAllocator::~SlabAllocator() {
    if (mem_base) mmap_free(mem_base);
}
