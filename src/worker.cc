...
#endif
{
    ...
    el = aeCreateEventLoop(conf.maxthreads + conf.maxclients +
                           EVENTLOOP_FDSET_INCR);

    // open the socket for listening to the connections from workers to exch
    // rdma resouces
    char neterr[ANET_ERR_LEN];
    char* bind_addr = conf.worker_bindaddr.length() == 0
                          ? nullptr
                          : const_cast<char*>(conf.worker_bindaddr.c_str());
    sockfd = anetTcpServer(neterr, conf.worker_port, bind_addr, conf.backlog);
    if (sockfd < 0) {
        epicLog(LOG_WARNING, "Opening port %d (bind_addr %s): %s",
                conf.worker_port, bind_addr, neterr);
        exit(1);
    }

    // register tcp event for rdma parameter exchange
    if (sockfd > 0 &&
        aeCreateFileEvent(el, sockfd, AE_READABLE, AcceptTcpClientHandle,
                          this) == AE_ERR) {
    ...
    if (resource->GetChannelFd() > 0 &&
        aeCreateFileEvent(el, resource->GetChannelFd(), AE_READABLE,
                          ProcessRdmaRequestHandle, this) == AE_ERR) {
        epicPanic("Unrecoverable error creating sockfd file event.");
    }
#endif
    ...
    if (htable == nullptr) epicPanic("Unable to allocate hash table!!!!");
#endif
    ...
    SyncMaster(
        FETCH_MEM_STATS);  // fetch the mem states of other workers from master
    ...
    epicLog(LOG_WARNING,
            "LRU eviction is enabled, max cache lines = %d, "
            "reserved mem size = %ld, cache_percentage = %f, block_size = %d",
            (int)(conf.size * conf.cache_th / BLOCK_SIZE), conf.size,
            conf.cache_th, BLOCK_SIZE);
#endif
...
void Worker::AsyncRdmaSendThread(Worker* w) {
    ...
    while (true) {
        while (w->rdma_queue->pop(data)) {
            epicLog(
                LOG_DEBUG,
                "To worker %d, buf = %p, len = %ld, id = %u, signaled = %d\n",
                data->client->GetWorkerId(), data->buf, data->len, data->id,
                data->signaled);
            int ret = data->client->Send(data->buf, data->len, data->id,
                                         data->signaled);
            delete data;
            data = nullptr;
            if (ret == -1) {
...
void Worker::StartService(Worker* w) {
    ...
    while (likely(!eventLoop->stop)) {
        ...
        while (!(ne = ibv_poll_cq(cq, MAX_CQ_EVENTS, wc)))
            ;
        if (ne < 0) {
            epicLog(LOG_WARNING, "poll CQ failed: %d:%s", errno,
                    strerror(errno));
        }
        ...
        while (w->wqueue->pop(wr)) {
            epicLog(LOG_DEBUG,
                    "wr->code = %d, wr->flag = %d, wr->addr = %lx, wr->size = "
                    "%d, wr->fd = %d\n",
        ...
        for (volatile int* buf : w->nbufs) {
            if (*buf == 1) {
                ...
                epicLog(LOG_DEBUG,
                        "wr->code = %d, wr->flag = %d, wr->addr = %lx, "
                        "wr->size = %d, wr->fd = %d\n",
                        wr->op, wr->flag, wr->addr, wr->size, wr->fd);
                if (wr->flag & ASYNC) {
                    *buf = 2;  // notify the app thread to return immediately
...
int Worker::RegisterHandle(int fd, aeFileProc* handle) {
    ...
    if (fd > 0 && aeCreateFileEvent(el, fd, AE_READABLE, ProcessLocalRequest,
                                    this) == AE_ERR) {
...
void Worker::ProcessToServeRequest(WorkRequest* wr) {
    ...
    if (to_serve_local_requests.count(block)) {
        ...
        queue<WorkRequest*> lq(
            *entry);  // copy it to local queue so that we don't need to hold
                      // the lock after that
        int size = entry->size();
        ...
        for (int i = 0; i < size; i++) {
            ...
            epicAssert(to_serve);
            epicLog(LOG_DEBUG, "processed to-serve local request %d",
                    to_serve->op);
            epicLog(LOG_DEBUG,
                    "wr->code = %d, wr->flag = %d, wr->addr = %lx, wr->size = "
                    "%d, wr->fd = %d, wr->wid = %d\n",
                    to_serve->op, to_serve->flag, to_serve->addr,
                    to_serve->size, to_serve->fd, to_serve->wid);
            epicAssert(to_serve->fd != 0 &&
    ...
    if (to_serve_requests.count(block)) {
        ...
        int size = entry->size();
        queue<pair<Client*, WorkRequest*>> lq(
            *entry);  // copy it to local queue so that we don't need to hold
                      // the lock after that
        ...
        for (int i = 0; i < size; i++) {
            ...
            epicLog(LOG_DEBUG,
                    "wr->code = %d, wr->flag = %d, wr->addr = %lx, wr->size = "
                    "%d, wr->fd = %d\n",
                    to_serve.second->op, to_serve.second->flag,
                    to_serve.second->addr, to_serve.second->size,
                    to_serve.second->fd);
#ifdef NOCACHE
            epicAssert(RLOCK == to_serve.second->op ||
                       WLOCK == to_serve.second->op);
#else
            epicAssert((READ == to_serve.second->op ||
                        READ_FORWARD == to_serve.second->op ||
...
#endif
            epicAssert(to_serve.first->GetWorkerId() != 0 &&
...
size_t Worker::GetWorkersSize() { return widCliMapWorker.size(); }
    ...
    if (unlikely(widCliMap.size() == 0)) {
        ...
        if (addr) {
            ...
            try {
                ...
                epicLog(LOG_WARNING,
                        "cannot find the client for worker %d (%s)", wid,
                        oor.what());
            }
        } else {
            ...
            epicLog(LOG_DEBUG,
                    "select the server with most free memory to allocate");
            Size max = 0;
            for (auto& entry : widCliMapWorker) {
                ...
                if (entry.first == GetWorkerId()) continue;
...
unsigned long long Worker::SubmitRequest(Client* cli, WorkRequest* wr, int flag,
    ...
    if (!((wr->op & REPLY) || (flag & REQUEST_NO_ID))) wr->id = GetWorkPsn();
    if (flag & ADD_TO_PENDING) AddToPending(wr->id, wr);
    ...
    if (!fence->in_process) {
        ...
        if (fence->pending_writes == 0) {
            if (fence->mfenced || fence->sfenced) {
                ...
                WorkRequest* lwr;
                bool fenced = false;
                while (fence->pending_works.size() &&
                       (lwr = fence->pending_works.front())) {
                    // epicAssert(wr->fd == lwr->fd);
                    epicLog(LOG_DEBUG,
                            "processed fenced (mfenced = %d, sfenced = %d) op "
                            "= %d, addr = %lx, fd = %d",
                            fence->mfenced, fence->sfenced, lwr->op, lwr->addr,
                            lwr->fd);
                    if (lwr->op == MFENCE) {
                        ...
                        fenced = true;
                        epicLog(LOG_DEBUG,
                                "mfenced due to RLOCK!!, pending_writes = %d",
                                fence->pending_writes.load());
                        // do not pop it from fence->pending_works
                        // otherwise the order of this request will be put in
                        ...
                        fenced = true;
                        epicLog(LOG_DEBUG,
                                "mfenced due to WLOCK!!, pending_writes = %d",
                                fence->pending_writes.load());
                        // do not pop it from fence->pending_works
                        // otherwise the order of this request will be put in
                        // the back of the queue
                        ...
                        fenced = true;
                        epicLog(LOG_DEBUG,
                                "mfenced due to UNLOCK!!, pending_writes = %d",
                                fence->pending_writes.load());
                        // do not pop it from fence->pending_works
                        // otherwise the order of this request will be put in
                        // the back of the queue
                    ...
                    //					//TODO: if sfenced, we should allow read to
                    //pass 					if(fence->mfenced || fence->sfenced) { 						break;
                    //					}
                }
                /*
                 * we must not change the fenced status until last
                 * since otherwise it may allow other thread to pass the fence
                 * and process request before the fenced requests are processed,
...
int Worker::Notify(WorkRequest* wr) {
    ...
    if (wr->flag & ASYNC) {
        epicAssert(wr->op == WRITE || wr->op == MFENCE ||
                   wr->op == UNLOCK);  // currently only writes, mfence and
                                       // unlock are asynchronous
        Fence* fence = fences_.at(wr->fd);
        epicLog(LOG_DEBUG, "Asynchronous request notification, pending %d",
                fence->pending_writes.load());

        if (WRITE == wr->op) fence->pending_writes--;
        if (fence->pending_writes < 0) {
        ...
        if (wr->flag & COPY) {
            WorkRequest* o = wr;
            wr = wr->Copy();
            if (wr->ptr && wr->size) zfree(wr->ptr);
            delete wr;
            wr = nullptr;
        }
    } else {
        ...
        if (write(wr->fd, "r", 1) != 1) {
            epicLog(LOG_WARNING, "writing to pipe error (%d:%s)", errno,
                    strerror(errno));
        }
...
#else
        // epicLog(LOG_WARNING, "wr->notify_buf = %d, wr = %lx, wr->notify_buf =
        // %lx", *wr->notify_buf, wr, wr->notify_buf);
        epicAssert(*wr->notify_buf == 1);
#endif
        *wr->notify_buf = 2;
        //	epicAssert(*wr->notify_buf == 2); //DO NOT check here, user
        //thread may already change its value!!!!
...
void Worker::AddToPending(unsigned int id, WorkRequest* wr) {
    epicAssert(!(wr->flag & ASYNC) || !(wr->flag & LOCAL_REQUEST) ||
               ((wr->flag & ASYNC) && (wr->flag & COPY)));
    epicAssert(id == wr->id);
    // LOCK_MICRO(pending_works, id);
...
WorkRequest* Worker::GetPendingWork(unsigned int id) {
    ...
    try {
        ...
        epicLog(LOG_WARNING, "cannot find the pending work %d (%s)", id,
                e.what());
        // UNLOCK_MICRO(pending_works, id);
        return nullptr;
    }
...
int Worker::GetAndErasePendingWork(unsigned int id, WorkRequest** wp) {
    ...
    try {
        *wp = pending_works.at(id);
        ...
        epicLog(LOG_FATAL, "cannot find the pending work %d (%s)", id,
                e.what());
        // UNLOCK_MICRO(pending_works, id);
        return 0;
    }
