...
// #if !defined(USE_PIPE_W_TO_H) || (!defined(USE_BOOST_QUEUE) &&
// !defined(USE_PIPE_H_TO_W))
...
WorkerHandle::~WorkerHandle() {
    DeRegisterThread();
#if !defined(USE_PIPE_W_TO_H) || \
    (!defined(USE_BOOST_QUEUE) && !defined(USE_PIPE_H_TO_W))
...
int WorkerHandle::SendRequest(WorkRequest* wr) {
    ...
    *notify_buf = 1;  // not useful to set it to 1 if boost_queue is enabled
...
#endif
    epicLog(LOG_DEBUG,
            "workid = %d, wr->notify_buf = %d, wr->op = %d, wr->flag = %d, "
            "wr->status = %d, wr->addr = %lx, wr->size = %d, wr->fd = %d",
            worker->GetWorkerId(), *wr->notify_buf, wr->op, wr->flag,
            wr->status, wr->addr, wr->size, wr->fd);
    int ret = worker->ProcessLocalRequest(
        wr);  // not complete due to remote or previously-sent similar requests
    if (ret) {  // not complete due to remote or previously-sent similar
                // requests
        if (wr->flag & ASYNC) {
            return SUCCESS;
        } else {
    ...
    if (wr->flag &
        ASYNC) {  // if the work request is asynchronous, we return immediately
...
#else
    *notify_buf = 1;  // put first since notify_buf = 1 may happen after
                      // notify_buf = 2 by worker
    epicAssert(*(int*)notify_buf == 1);
    to->fd = recv_pipe[1];  // for legacy code
    to->notify_buf = this->notify_buf;
    wqueue->push(to);
...
#ifndef USE_BUF_ONLY  // otherwise, we have to return after the worker copy the
                      // data from notify_buf
    // we return directly without writing to the pipe to notify the worker
    // thread
    if (to->flag & ASYNC) {
...
void WorkerHandle::ReportCacheStatistics() {
    ...
    fprintf(
        stdout, "%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\n",
        worker->no_local_reads_.load(), worker->no_local_reads_hit_.load(),
        worker->no_local_writes_.load(), worker->no_local_writes_hit_.load(),
        worker->no_remote_reads_.load(), worker->no_remote_reads_hit_.load(),
        worker->no_remote_writes_.load(), worker->no_remote_writes_hit_.load(),
