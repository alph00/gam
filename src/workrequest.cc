...
WorkRequest::WorkRequest(WorkRequest& wr) {
    ...
    pid = wr.pid;  // identifier of the parent work request (used for FORWARD
                   // request)
    ...
    gfunc = wr.gfunc;
    arg = wr.arg;
    if (flag & GFUNC) epicAssert(gfunc);
#endif
}

bool WorkRequest::operator==(const WorkRequest& wr) {
    return wr.addr == this->addr && wr.counter == this->counter &&
           wr.fd == this->fd && wr.flag == this->flag &&
           wr.free == this->free && wr.id == this->id &&
           wr.next == this->next && wr.op == this->op &&
