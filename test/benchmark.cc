...
int time_locality =
    10;  // 0..100 (how probable it is to re-visit the current position)
...
int CyclingIncr(int a, int cycle_size) { return ++a == cycle_size ? 0 : a; }
...
void PopulateOneBlock(GAlloc* alloc, GAddr data[], GAddr* ldata[], int i,
    ...
    for (int j = 0; j < items_per_block; j++) {
        ...
        if (j + 1 == items_per_block) {
            // next = GADD(data[GetRandom(0, STEPS, seedp)], GetRandom(0,
            // items_per_block, seedp)*item_size); //data[CyclingIncr(i,
            // STEPS)];
            ...
            if (TrueOrFalse(space_locality, seedp)) {
                ...
                // STEPS)];
                GAddr n =
                    GADD(data[GetRandom(0, STEPS, seedp)],
                         GetRandom(0, items_per_block, seedp) * item_size);
                while (TOBLOCK(n) == TOBLOCK(data[i])) {
                    n = GADD(data[GetRandom(0, STEPS, seedp)],
                             GetRandom(0, items_per_block, seedp) * item_size);
        ...
        *((GAddr*)data[i] + j) = next;
#else
        //		int ret = alloc->Write(data[i], j*item_size, &next,
        //item_size); 		epicAssert(ret == item_size);
        ldata[i][j] = next;
#endif
...
void Init(GAlloc* alloc, GAddr data[], GAddr access[], bool shared[], int id,
    ...
    if (is_master && id == 0) {
...
#endif
            if (shared_ratio != 0) alloc->Put(i, &data[i], addr_size);
        ...
        for (int i = 0; i < STEPS; i++) {
            ...
            if (TrueOrFalse(l_shared_ratio, seedp)) {
                ...
                data[i] = addr;
                // revise the l_remote_ratio accordingly if we get the shared
                // addr violate the remote probability
                if (TrueOrFalse(l_remote_ratio, seedp)) {  // should be remote
                    if (alloc->GetID() == WID(addr)) {     // false negative
                        l_remote_ratio =
                            Revise(l_remote_ratio, STEPS - i - 1, false);
                    }
                } else {                                // shouldn't be remote
                    if (alloc->GetID() != WID(addr)) {  // false positive
                        l_remote_ratio =
                            Revise(l_remote_ratio, STEPS - i - 1, true);
                    }
                }
                shared[i] = true;
            } else {
#ifdef LOCAL_MEMORY
                int ret =
                    posix_memalign((void**)&data[i], BLOCK_SIZE, BLOCK_SIZE);
                epicAssert(data[i] % BLOCK_SIZE == 0);
#else
                if (TrueOrFalse(remote_ratio, seedp)) {
                    data[i] = alloc->AlignedMalloc(BLOCK_SIZE, REMOTE);
    ...
    for (int i = 1; i < ITERATION; i++) {
        // PopulateOneBlock(alloc, data, ldata, i, l_remote_ratio,
        // l_space_locality, seedp);
        GAddr next;
        if (TrueOrFalse(space_locality, seedp)) {
...
//				writes++;
            ...
            case 3:  // try_rlock/try_wlock
            {
                ...
                if (!lret) alloc->UnLock(to_access, item_size);
                break;
            }
            default:
    ...
    if (op_type == 0) {
        // issue a fence and a read request to the last address to ensure all
        // previous op have been done
        alloc->MFence();
        ret = alloc->Read(to_access, buf, item_size);
    }
    ...
    epicLog(LOG_WARNING,
            "node_id %d, thread %d, average throughput = %ld per-second, "
            "latency = %ld ns %s",
...
void Benchmark(int id) {
    ...
    epicLog(
        LOG_WARNING, "Malloc (local): throughput = %lf op/s, latency = %ld ns",
        (double)it / ((double)duration / 1000 / 1000 / 1000), duration / it);
    ...
    //	end = get_time();
    //	duration = end - start;
    //	epicLog(LOG_WARNING, "Free (local): throughput = %lf op/s, latency = %ld
    //ns", 			(double )it / ((double )duration / 1000 / 1000 / 1000), 			duration /
    //it);
    ...
    duration = end - start;
    epicLog(
        LOG_WARNING, "Malloc (remote): throughput = %lf op/s, latency = %ld ns",
        (double)it / ((double)duration / 1000 / 1000 / 1000), duration / it);
...
//	end = get_time();
//	duration = end - start;
//	epicLog(LOG_WARNING, "Free (remote): throughput = %lf op/s, latency =
//%ld ns", 			(double )it / ((double )duration / 1000 / 1000 / 1000), 			duration /
//it);
#endif
    ...
    //				//FIXME:remove
    //				epicAssert(!(id == 0 && is_master)); //cannot be the master
    //process 				epicAssert(ldata[i][j] == TOBLOCK(ldata[i][j])); //should be the
    //starting addr of the next block in the master access trace
    //				//ldata[i][j] = data[GetRandom(0, STEPS, &seedp)];
    ////data[CyclingIncr(i, STEPS)];  //update to the starting addr of the next
    //block in its private access trace 				GAddr n = GADD(data[GetRandom(0, STEPS,
    //&seedp)], GetRandom(0, items_per_block, &seedp)*item_size);
    //				while(TOBLOCK(n) == TOBLOCK(data[i])) { //data[CyclingIncr(i,
    //STEPS)]; 					n = GADD(data[GetRandom(0, STEPS, &seedp)], GetRandom(0,
    //items_per_block, &seedp)*item_size);
    //				}
    //				ldata[i][j] = n;
    //			}
    //		}
    ...
    for (int i = 1; i <= no_node; i++) {
        for (int j = 0; j < no_thread; j++) {
            ...
            epicLog(LOG_WARNING, "get sync_id %d from node %d, thread %d",
                    sync_id, i, j);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    printf(
        "master: %s:%d, worker: %s:%d, is_master: %s, no_thread: %d, no_node: "
        "%d\n",
    ...
    printf(
        ...
        "read_ratio: %d, "
        "space_locality: %d, time_locality: %d, op_type = %s, memory_type = "
        "%s, item_size = %d, cache_th = %f, result_file = %s\n",
        no_node, no_thread, remote_ratio, shared_ratio, read_ratio,
        space_locality, time_locality,
        op_type == 0 ? "read/write"
                     : (op_type == 1 ? "rlock/wlock"
                                     : (op_type == 2 ? "rlock+read/wlock+write"
                                                     : "try_rlock/try_wlock")),
        memory_type == 0 ? "local memory" : "global memory", item_size,
        cache_th, result_file);
    ...
    a_lat /= no_thread;
    epicLog(LOG_WARNING,
            "results for node_id %d: total_throughput: %ld, "
            "avg_throuhgput:%ld, avg_latency:%ld",
    ...
    if (is_master) {
        ...
        result << no_node << "," << no_thread << "," << remote_ratio << ","
               ...
               << "," << item_size << "," << t_thr << "," << a_thr << ","
               << a_lat << "," << cache_th << "\n";
        epicLog(LOG_WARNING,
                "results for all the nodes: "
                "no_node: %d, no_thread: %d, remote_ratio: %d, shared_ratio: "
                "%d, read_ratio: %d, space_locality: %d, "
                "time_locality: %d, op_type = %d, memory_type = %d, item_size "
                "= %d, "
                "total_throughput: %ld, avg_throuhgput:%ld, avg_latency:%ld, "
                "cache_th = %f\n\n",
                no_node, no_thread, remote_ratio, shared_ratio, read_ratio,
...
#ifdef STATS_COLLECTION
    epicLog(LOG_WARNING,
            "shared_ratio = %lf, remote_ratio = %lf, read_ratio = %lf, "
            "space_locality = %lf, time_locality = %lf, "
            "total blocks touched %d, expected blocks touched %d\n",
            ((double)shared_access) / (ITERATION * no_thread) * 100 / 2,
            ((double)remote_access) / (ITERATION * no_thread) * 100 / 2,
            ((double)read_access) / (ITERATION * no_thread) * 100 / 2,
            ((double)space_local_access) / (ITERATION * no_thread) * 100 / 2,
            ((double)time_local_access) / (ITERATION * no_thread) * 100 / 2,
