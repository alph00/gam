...
std::string generateKey<std::string>(size_t i) {
    ...
    if (num.size() >= min_length) return num;
    std::string ret(min_length, 'a');
    const size_t startret = min_length - num.size();
    for (size_t i = 0; i < num.size(); ++i) ret[i + startret] = num[i];
    return ret;
}
...
class insert_thread_no_lock {
   public:
    typedef typename std::vector<typename Table::key_type>::iterator it_t;
    static void func(Table& table, it_t begin, it_t end) {
        for (; begin != end; ++begin) assert(table.insert(*begin, 0));
    }
};
    ...
    static void func(Table& table, it_t begin, it_t end,
        ...
        while (inserter_it != end) {
            if (dist(gen) < insert_prob) {
                ...
                ++reader_it;
                if (reader_it == end) reader_it = begin;
            }
            ++ops;
        }
   ...
   public:
    TestEnvironment()
        ...
        for (size_t i = 0; i < thread_num; ++i)
            threads.emplace_back(insert_thread_no_lock<T>::func,
                                 std::ref(table),
...
void ReadThroughputTest(TestEnvironment<T>* env) {
    ...
    const size_t out_keys_per_thread =
        (env->numkeys - env->init_size) / second_readnum;
...
int main(int argc, char** argv) {
    ...
    const char* arg_help[] = {
        ...
        "Choice of test mode: 0 for read, 1 for insert, 2 for mix of read & "
        "insert"};
