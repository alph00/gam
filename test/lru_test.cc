...
void thread1_1() {
    ...
    for (int j = 0; j < block; j++) {
        ...
        for (i = 0; i < size; i++) {
            if (buf[i] != wbuf[i]) {
                epicLog(LOG_WARNING,
                        "local read failed at buf[%d] (%d) != %d\n", i, buf[i],
                        wbuf[i]);
                break;
            }
        }
        if (i == size) {
            epicLog(LOG_WARNING,
                    "*****local write (thread1_1 %d) succeed buf[0] = %d!*****",
                    j, buf[0]);
        } else {
...
void thread2_1() {
    ...
    for (int j = 0; j < block; j++) {
        ...
        for (i = 0; i < size; i++) {
            if (buf[i] != wbuf[i]) {
                epicLog(LOG_WARNING,
                        "remote read failed at buf[%d] (%d) != %d\n", i, buf[i],
                        wbuf[i]);
                break;
            }
        }
        if (i == size) {
            epicLog(
                LOG_WARNING,
                "*****remote write succeed (thread2_1 %d), buf[0] =  %d!*****",
                j, buf[0]);
        } else {
...
void thread3_1() {
    ...
    for (int j = 0; j < block; j++) {
        ...
        if (i == size) {
            epicLog(LOG_WARNING,
                    "*****local write (thread3_1 %d) succeed buf[0] = %d!*****",
                    j, buf[0]);
        } else {
