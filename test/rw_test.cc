...
int main() {
    ...
    for (i = 0; i < size; i++) {
        if (buf[i] != wbuf[i]) {
            epicLog(LOG_WARNING,
                    "READ Case 2 (null): remote read failed at buf[%d] (%d) != "
                    "%d\n",
                    i, buf[i], wbuf[i]);
            break;
        }
    ...
    for (i = 0; i < size; i++) {
        if (buf[i] != wbuf[i]) {
            epicLog(LOG_WARNING,
                    "READ Case 2 (shared): remote read failed at buf[%d] (%d) "
                    "!= %d\n",
                    i, buf[i], wbuf[i]);
            break;
        }
    ...
    for (i = 0; i < size; i++) {
        if (buf[i] != wbuf[i]) {
            epicLog(LOG_WARNING,
                    "WRITE Case 3 (shared, cached): remote read failed at "
                    "buf[%d] (%d) != %d\n",
                    i, buf[i], wbuf[i]);
            break;
        }
    ...
    {  // (WRITE Case 1, dflag = shared)
        ...
        for (i = 0; i < size; i++) {
            if (buf[i] != wbuf[i]) {
                epicLog(LOG_WARNING,
                        "WRITE Case 1 - step 1: remote read failed at buf[%d] "
                        "(%d) != %d\n",
                        i, buf[i], wbuf[i]);
                break;
            }
    ...
    sleep(1);
    {
        ...
        for (i = 0; i < size; i++) {
            if (buf[i] != wbuf[i]) {
                epicLog(LOG_WARNING,
                        "WRITE Case 2 - step 1: remote read failed at buf[%d] "
                        "(%d) != %d\n",
                        i, buf[i], wbuf[i]);
                break;
            }
    ...
    sleep(1);
    {
        ...
        for (i = 0; i < size; i++) {
            if (buf[i] != wbuf[i]) {
                epicLog(LOG_WARNING,
                        "WRITE Case 3 (shared, non-cached) - step 1: remote "
                        "read failed at buf[%d] (%d) != %d\n",
                        i, buf[i], wbuf[i]);
                break;
            }
        }
        ...
        for (i = 0; i < size; i++) {
            if (i == size - 1 && buf[i] == (char)(wbuf[i] - 1)) continue;
            if (buf[i] != wbuf[i]) {
                epicLog(LOG_WARNING,
                        "WRITE Case 3 (shared, non-cached): remote read failed "
                        "at buf[%d] (%d) != %d\n",
                        i, buf[i], wbuf[i]);
                break;
            }
    ...
    sleep(1);
    {
        ...
        for (i = 0; i < size; i++) {
            if (i == 0 && buf[i] == (char)(wbuf[i] - 1)) continue;
            if (buf[i] != wbuf[i]) {
                epicLog(LOG_WARNING,
                        "WRITE Case 3 (shared, cached): remote read failed at "
                        "buf[%d] (%d) != %d\n",
